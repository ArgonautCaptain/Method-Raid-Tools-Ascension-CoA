...
local MRT_DEFAULT_FONT = (GameFontNormal and GameFontNormal.GetFont and select(1,GameFontNormal:GetFont())) or "Fonts\\FRIZQT__.TTF"
...
do
	...
	if ok and f then
		...
		if type(idx) == "table" then
			...
			if not idx.SetEnabled then
				function idx:SetEnabled(state)
					if state then
						if self.Enable then return self:Enable() end
					else
						if self.Disable then return self:Disable() end
					end
				end
			end
		end
...
if not ExRTFontNormal then
		_G.ExRTFontNormal = CreateFont("ExRTFontNormal")

	ExRTFontNormal:CopyFontObject(GameFontNormal)
	local _fp, _fs, _ff = ExRTFontNormal:GetFont()
	if not _fp then
		local path, size, flags = GameFontNormal:GetFont()
		ExRTFontNormal:SetFont(path or STANDARD_TEXT_FONT, size or 12, flags)
	end
	ExRTFontNormal:SetShadowColor(0,0,0)
	ExRTFontNormal:SetShadowOffset(1,-1)
	_G.ExRTFontNormal:SetTextColor(1,.82,0)
end
if not ExRTFontGrayTemplate then
		_G.ExRTFontGrayTemplate = CreateFont("ExRTFontGrayTemplate")
	...
	_G.ExRTFontGrayTemplate:SetTextColor(0.63,0.68,0.69)
end


do
	local ICONS = {
		[1] = {{0.5,0.5625,0.5,0.625},{1,1,1,.7},{.8,0,0,1}},
		[2] = {{0.1875,0.25,0.5,0.625},{1,1,1,.7},{0.9,0.75,0,1}},
		[3] = {{0.25,0.3125,0.5,0.625},{1,1,1,.7},0,{1,1,1,1},{.3,.3,.3,.7}},
		[4] = {{0.3125,0.375,0.5,0.625},{1,1,1,.7},0,{1,1,1,1},{.3,.3,.3,.7}},
	...
	function Templates:Border(self,cR,cG,cB,cA,size,offsetX,offsetY)
		...
		SetSolidColor(self.BorderTop,cR,cG,cB,cA)
		...
		SetSolidColor(self.BorderLeft,cR,cG,cB,cA)
		...
		SetSolidColor(self.BorderBottom,cR,cG,cB,cA)
		...
		SetSolidColor(self.BorderRight,cR,cG,cB,cA)
...
function Templates:ExRTTranslucentFrameTemplate(parent)
	...
	SetSolidColor(self.Bg,0,0,0,0.8)
...
do
	...
	local function create_slider(self)
		...
		SetSolidColor(self.slider.thumb,.32,.32,.4,1)
		...
		if self.slider and self.slider.SetObeyStepOnDrag then
...
do
	...
	local function OnHide(self)
		...
		self._logicalShown = false
		if self.Level and ELib.ScrollDropDown and ELib.ScrollDropDown.DropDownList then
			for i = (self.Level + 1), #ELib.ScrollDropDown.DropDownList do
				local deeper = ELib.ScrollDropDown.DropDownList[i]
				if deeper then
					if ELib.ScrollDropDown._logicalHide then
						ELib.ScrollDropDown._logicalHide(deeper)
					elseif deeper.Hide then
						deeper:Hide()
					end
				end
			end
...
function Templates:ExRTButtonTransparentTemplate(parent,isSecure)
	...
	self.HighlightTexture = self:CreateTexture()
	SetSolidColor(self.HighlightTexture,1,1,1,.3)
	self.HighlightTexture:SetPoint("TOPLEFT")
	self.HighlightTexture:SetPoint("BOTTOMRIGHT")
	self:SetHighlightTexture(self.HighlightTexture)

	self.PushedTexture = self:CreateTexture()
	SetSolidColor(self.PushedTexture,.9,.8,.1,.3)
	self.PushedTexture:SetPoint("TOPLEFT")
	self.PushedTexture:SetPoint("BOTTOMRIGHT")
	self:SetPushedTexture(self.PushedTexture)
...
function Templates:ExRTButtonModernTemplate(parent,isSecure)
	...
	SetSolidColor(self.Texture,1,1,1,1)
	...
	self.DisabledTexture = self:CreateTexture()
	SetSolidColor(self.DisabledTexture,0.20,0.21,0.25,0.5)
	self.DisabledTexture:SetPoint("TOPLEFT")
	self.DisabledTexture:SetPoint("BOTTOMRIGHT")
	self:SetDisabledTexture(self.DisabledTexture)
...
do
	...
	function Templates:ExRTDropDownButtonModernTemplate(parent)
		...
		self.Background = self:CreateTexture(nil,"BACKGROUND")
		SetSolidColor(self.Background,0,0,0,.3)
		self.Background:SetPoint("TOPLEFT")
		self.Background:SetPoint("BOTTOMRIGHT")
...
do
	...
	function Templates:ExRTInputBoxTemplate(parent)
		...
		if self.SetFont and _G.GameFontWhite and _G.GameFontWhite.GetFont then
			local _fp, _fs, _ff = _G.GameFontWhite:GetFont()
			if _fp then self:SetFont(_fp, _fs or 12, _ff) end
		end
		if self.SetTextColor then self:SetTextColor(1,1,1,1) end
...
do
	...
	function Templates:ExRTSliderModernTemplate(parent)
		...
		self.Thumb = self:CreateTexture()
		SetSolidColor(self.Thumb,0.44,0.45,0.50,0.7)
...
do
	...
	function Templates:ExRTTrackingButtonModernTemplate(parent)
		...
		SetSolidColor(self.IconOverlay,0,0,0,0.5)
...
function Templates:ExRTCheckButtonModernTemplate(parent)
	...
	self.Texture = self:CreateTexture(nil,"BACKGROUND")
	SetSolidColor(self.Texture,0,0,0,.3)
	self.Texture:SetPoint("TOPLEFT")
	self.Texture:SetPoint("BOTTOMRIGHT")
...
do
	...
	local function Widget_Texture(self,texture,cG,cB,cA)
		if cG then
			SetSolidColor(self,texture,cG,cB,cA)
		else
	...
	function ELib:Texture(parent,cR,cG,cB,cA,layer)
		...
		if texture then
			self:SetTexture(texture)
		elseif cR then
			SetSolidColor(self,cR,cG,cB,cA)
		end
...
do
	function ELib:ShadowInside(mainFrame,enableBorder,enableLine)
		local function SA(t,a,b)
			if not t then return end
			if type(t.SetAtlas)=="function" then
				local ok=pcall(t.SetAtlas,t,a,b)
		...
		if enableBorder then
			...
			SA(OverlayShadowTopLeft,"collections-background-shadow-small",true)
			...
			SA(OverlayShadowTop,"collections-background-shadow-small",true)
			...
			SA(OverlayShadowTopRight,"collections-background-shadow-small",true)
			...
			SA(OverlayShadowLeft,"collections-background-shadow-small",true)
			...
			SA(OverlayShadowBottomLeft,"collections-background-shadow-small",true)
			...
			SA(OverlayShadowRight,"collections-background-shadow-small",true)
			...
			SA(OverlayShadowBottomRight,"collections-background-shadow-small",true)
			...
			SA(OverlayShadowBottom,"collections-background-shadow-small",true)
		...
		if enableLine then
			...
			SA(ShadowLineTop,"_collections-background-line",true)
			...
			SA(ShadowLineBottom,"_collections-background-line",true)
		...
		SA(ShadowCornerTopLeft,"collections-background-shadow-large",true)

		local ShadowCornerTopRight = mainFrame:CreateTexture(nil,"BORDER",nil,2)
		ShadowCornerTopRight:SetPoint("TOPRIGHT",-offset,-offset)
		SA(ShadowCornerTopRight,"collections-background-shadow-large",true)
		...
		SA(ShadowCornerBottomLeft,"collections-background-shadow-large",true)
		...
		SA(ShadowCornerBottomRight,"collections-background-shadow-large",true)
		...
		SA(ShadowCornerTop,"collections-background-shadow-large",true)
		...
		SA(ShadowCornerLeft,"collections-background-shadow-large",true)
		...
		SA(ShadowCornerRight,"collections-background-shadow-large",true)
		...
		SA(ShadowCornerBottom,"collections-background-shadow-large",true)
...
do
	...
	local function Widget_OnChange(self,func)
		if type(func) == "function" then
			local orig = func
			func = function(s, value, ...)
				local step = s.GetValueStep and s:GetValueStep() or 1
				if not step or step <= 0 then step = 1 end
				local snapped = math.floor((value / step) + 0.5) * step
				if s.__MRT_lastStep == snapped then return end
				s.__MRT_lastStep = snapped
				return orig(s, snapped, ...)
			end
		end
...
do
	...
	local function Widget_Range(self,minVal,maxVal,clickRange,unchangedValue)
			if type(minVal) ~= "number" or minVal ~= minVal then minVal = 0 end
			if type(maxVal) ~= "number" or maxVal ~= maxVal then maxVal = minVal end
	...
	local function Widget_SetMinimal(self)
		...
		if not self.isOld then
			if not self.borderTop then
				...
				SetSolidColor(self.borderTop,0.24,0.25,0.30,1)
			end
			if not self.borderBottom then
				...
				SetSolidColor(self.borderBottom,0.24,0.25,0.30,1)
			end
		end
	...
	function ELib:ScrollBar(parent,isOld)
		...
		SetSolidColor(self.bg,0, 0, 0, 0.3)
		if not isOld then
			self.thumb = self.slider:CreateTexture(nil, "OVERLAY")
			SetSolidColor(self.thumb,0.44,0.45,0.50,.7)
		...
		if not isOld then
			...
			SetSolidColor(self.borderLeft,0.24,0.25,0.30,1)
			...
			SetSolidColor(self.borderRight,0.24,0.25,0.30,1)
		end
...
do
	...
	local function CreateAdditionalTooltip()
		...
		SetSolidColor(tip.gradientTexture,1,1,1,1)
...
do
	...
	function ELib:Tabs(parent,template,...)
		...
		for i=1,tabCount do
			...
			if self.tabs[i].EnableMouseWheel then
				self.tabs[i]:EnableMouseWheel(true)
			end
...
do
	...
	local function OnTooltipEnter(self)
		...
		if text.extraTip and not (text.IsTruncated and text:IsTruncated()) then
		...
		if not (text.IsTruncated and text:IsTruncated()) and not text.alwaysTooltip then
			return
		end
		if type(text.extraTip) == "function" then
	...
	function ELib:Text(parent,text,size,template)
		...
		if template and type(template) == "string" and not _G[template] then
			template = nil
		end

		local self = parent:CreateFontString(nil,"ARTWORK",template)
		local filename = self:GetFont()
		if not filename and GameFontNormal and self.SetFontObject then
			self:SetFontObject(GameFontNormal)
			filename = self:GetFont()
		end
		if not filename and self.SetFont then
			self:SetFont(STANDARD_TEXT_FONT or "Fonts\\FRIZQT__.TTF", size or 12, "")
			filename = self:GetFont()
		end
...
do
	...
	local function Widget_FontSize(self,size)
		local filename,fontSize,fontParam1,fontParam2,fontParam3 = self:GetFont()
		local fontObj = GetFontForSize(size)
		if fontObj then
			self:SetFontObject(fontObj)
			if self.SetFont then
				local _src = _G[fontObj] or fontObj
				if type(_src) == "table" and _src.GetFont then
					local _fp,_fs,_ff = _src:GetFont()
					if _fp then self:SetFont(_fp, _fs or size or 12, _ff) end
				end
			end
		else
			self:SetFont(filename,size,fontParam1,fontParam2,fontParam3)
	...
	function ELib:Edit(parent,maxLetters,onlyNum,template)
		...
		if self.SetTextColor then
			...
			if not template then
				local _fontSrc = ExRTFontNormal or GameFontNormal
				if _fontSrc then
					self:SetFontObject(_fontSrc)
				end
				if self.SetFont and _fontSrc and _fontSrc.GetFont then
					local _fp, _fs, _ff = _fontSrc:GetFont()
					if not _fp and GameFontNormal and GameFontNormal.GetFont then
						_fp, _fs, _ff = GameFontNormal:GetFont()
					end
					if _fp then self:SetFont(_fp, _fs or 12, _ff) end
				end
			self:SetTextColor(1,1,1,1)
...
do
	...
	local function ScrollFrameChangeHeight(self,newHeight)
			if type(newHeight) ~= "number" or newHeight ~= newHeight then
				newHeight = 0
			end
	...
	local function ScrollFrameChangeWidth(self,newWidth)
			if type(newWidth) ~= "number" or newWidth ~= newWidth then
				newWidth = 0
			end
...
do
	...
	local function Widget_SetVertical(self)
		...
		local _, _, _, uiV = GetBuildInfo()
		uiV = tonumber(uiV) or 0
		if uiV > 0 and uiV <= 30300 then
			local label = (self.GetText and self:GetText()) or ""
			if label ~= "" then
				for ri = 1, self:GetNumRegions() do
					local r = select(ri, self:GetRegions())
					if r and r.GetText and r.GetText(r) == label then
						r:SetAlpha(0)
						r:Hide()
					end
				end

				self.__verticalChars = self.__verticalChars or {}
				for i = 1, #self.__verticalChars do
					self.__verticalChars[i]:Hide()
					self.__verticalChars[i]:SetText("")
				end

				local utf8sub = (ExRT and ExRT.F and ExRT.F.utf8sub) or function(s,a,b) return string.sub(s,a,b) end
				local utf8len = (ExRT and ExRT.F and ExRT.F.utf8len) or string.len
				local n = utf8len(label)
				local fontPath, fontSize = self.TextObj:GetFont()
				fontSize = (type(fontSize) == "number" and fontSize) or 12
				local lineH = fontSize + 1
				local btnH = self:GetHeight() or (n * lineH)
				local pad = math.max(2, (btnH - n * lineH) / 2)
				for c = 1, n do
					local fs = self.__verticalChars[c]
					if not fs then
						...
						self.__verticalChars[c] = fs
					end
					fs:ClearAllPoints()
					fs:SetPoint("TOP", self, "TOP", 0, -pad - (c - 1) * lineH)
					fs:SetText(utf8sub(label, c, c))
					fs:Show()
				end

				if not self.__verticalReassertHooked then
					local function ReassertHide()
						for ri = 1, self:GetNumRegions() do
							local r = select(ri, self:GetRegions())
							if r and r.GetText and r.GetText(r) == label then
								r:SetAlpha(0)
					...
					self:HookScript("OnEnter", ReassertHide)
					self:HookScript("OnLeave", ReassertHide)
					self:HookScript("OnMouseDown", ReassertHide)
					self:HookScript("OnMouseUp", ReassertHide)
					self:HookScript("OnShow", ReassertHide)
					self.__verticalReassertHooked = true
				end
			end
			return self
		end
...
do
	local function Widget_Icon(self,texture,cG,cB,cA)
		if cG then
			SetSolidColor(self.texture,texture,cG,cB,cA)
		else
...
do
	...
	local function Widget_ColorState(self,isBorderInsteadText)
		if isBorderInsteadText then
			...
			SetSolidColor(self.BorderTop,cR,cG,cB,1)
			SetSolidColor(self.BorderLeft,cR,cG,cB,1)
			SetSolidColor(self.BorderBottom,cR,cG,cB,1)
			SetSolidColor(self.BorderRight,cR,cG,cB,1)
		...
		if mainFrame.EnableHoverAnimation then
			if not self.anim then
				...
				self.anim.timer:SetScript("OnUpdate", function(self,elapsed)
					...
					SetSolidColor(self.line.AnimTexture,cR,cG,cB,cA)
				end)
				...
				SetSolidColor(self.AnimTexture,self.anim.timer.cR, self.anim.timer.cG, self.anim.timer.cB, self.anim.timer.cA)
			end
		...
		for i=val,#self.L do
			...
			if self.colors then
				if self.colors[i] then
					SetSolidColor(line.background,unpack(self.colors[i]))
				else
					SetSolidColor(line.background,0,0,0,0)
				end
			end
			line:Show()
			line.index = i
...
do
	function ELib.CreateColorPickButton(parent,width,height,relativePoint,x,y,cR,cG,cB,cA)
		...
		SetSolidColor(self.color,cR or 0, cG or 0, cB or 0, cA or 1)
...
local function _logicalShow(d)
	if not d:IsShown() then d:Show() end
	d:SetAlpha(1)
	d:EnableMouse(true)
	if d.EnableMouseWheel then d:EnableMouseWheel(true) end
	if d.border and d.border.SetAlpha then d.border:SetAlpha(1) end
	if d.Slider and d.Slider.EnableMouse then d.Slider:EnableMouse(true) end
	if d.Buttons then
		for i=1,(d.MaxLines or 0) do
			local b = d.Buttons[i]
			if b then
				if b.EnableMouse then b:EnableMouse(true) end
				if b.SetAlpha then b:SetAlpha(1) end
				if b.NormalText and b.NormalText.SetAlpha then b.NormalText:SetAlpha(1) end
				if b.Icon and b.Icon.SetAlpha then b.Icon:SetAlpha(1) end
				if b.Arrow and b.Arrow.SetAlpha then b.Arrow:SetAlpha(1) end
				if b.Texture and b.Texture.SetAlpha then b.Texture:SetAlpha(1) end
				if b.checkButton and b.checkButton.SetAlpha then b.checkButton:SetAlpha(1) end
				if b.radioButton and b.radioButton.SetAlpha then b.radioButton:SetAlpha(1) end
			end
		end
	end
	d._logicalShown = true
end
local function _logicalHide(d)
	if d:IsShown() then
		d:SetAlpha(0)
		d:EnableMouse(false)
		if d.EnableMouseWheel then d:EnableMouseWheel(false) end
		if d.border and d.border.SetAlpha then d.border:SetAlpha(0) end
		if d.Slider and d.Slider.EnableMouse then d.Slider:EnableMouse(false) end
		if d.Buttons then
			for i=1,(d.MaxLines or 0) do
				local b = d.Buttons[i]
				if b then
					...
					if b.NormalText and b.NormalText.SetAlpha then b.NormalText:SetAlpha(0) end
					if b.Icon and b.Icon.SetAlpha then b.Icon:SetAlpha(0) end
					if b.Arrow and b.Arrow.SetAlpha then b.Arrow:SetAlpha(0) end
					if b.Texture and b.Texture.SetAlpha then b.Texture:SetAlpha(0) end
					if b.checkButton and b.checkButton.SetAlpha then b.checkButton:SetAlpha(0) end
					if b.radioButton and b.radioButton.SetAlpha then b.radioButton:SetAlpha(0) end
				end
			end
		end
	end
	d._logicalShown = false
end
ELib.ScrollDropDown._logicalShow = _logicalShow
ELib.ScrollDropDown._logicalHide = _logicalHide

function ELib.ScrollDropDown.IsOpen()
	local d = ELib.ScrollDropDown.DropDownList
	d = d and d[1]
	if not d then return false end
	if d._logicalShown ~= nil then return d._logicalShown end
	return d:IsShown() or false
end
...
local function CreateDropDown(level)
	...
	dropDown:SetScript("OnEnter", function(self) UIDropDownMenu_StopCounting(self) end)
	dropDown:SetScript("OnLeave", function(self) UIDropDownMenu_StartCounting(self) end)
	dropDown.border = ELib:Shadow(dropDown,20)
	if dropDown.border and dropDown.border.EnableMouse then
		dropDown.border:EnableMouse(false)
	end
	dropDown.Buttons = {}
	dropDown.MaxLines = 0
	dropDown.isModern = true
	dropDown.Level = level
		if dropDown.BorderTop then
			SetSolidColor(dropDown.BorderTop,0.24,0.25,0.30,1)
			SetSolidColor(dropDown.BorderLeft,0.24,0.25,0.30,1)
			SetSolidColor(dropDown.BorderBottom,0.24,0.25,0.30,1)
			SetSolidColor(dropDown.BorderRight,0.24,0.25,0.30,1)
		end
...
do
	local PRECREATE_LEVELS = {2, 3, 4}
	local PRECREATE_BUTTONS = 30
	local function _prewarmModernDropDownLevels()
		if VMRT and VMRT.DebugReminder and VMRT.DebugReminder.SkipDropDownPrewarmL2L4 then
			return
		end
		local _prof = VMRT and VMRT.DebugReminder and VMRT.DebugReminder.ProfileShowStrata
		local _t0 = _prof and debugprofilestop and debugprofilestop()
		for _, level in ipairs(PRECREATE_LEVELS) do
			if not ScrollDropDown_Modern[level] then
				local d = CreateDropDown(level)
				for i = 1, PRECREATE_BUTTONS do
					ELib.ScrollDropDown.CreateButton(i, level, d)
					local b = d.Buttons[i]
					if b and b.Hide then b:Hide() end
				end
				d.MaxLines = PRECREATE_BUTTONS
				d:ClearAllPoints()
				d:SetPoint("TOPLEFT", UIParent, "TOPLEFT", -9999, -9999)
				d:SetAlpha(0)
				d:EnableMouse(false)
				if d.EnableMouseWheel then d:EnableMouseWheel(false) end
				if d.border and d.border.SetAlpha then d.border:SetAlpha(0) end
				if d.Slider then
					if d.Slider.EnableMouse then d.Slider:EnableMouse(false) end
					if d.Slider.Hide then d.Slider:Hide() end
				end
				d:Show()
				d._logicalShown = false
			end
		end
		if _prof and _t0 then
			print(string.format(
				"|cff33ff99[dropdown-prewarm]|r L2..L4 created+shown = %.1fms",
				debugprofilestop() - _t0
			))
		end
	end
	if C_Timer and C_Timer.After then
		C_Timer.After(0.5, _prewarmModernDropDownLevels)
	else
		_prewarmModernDropDownLevels()
	end
end
...
for i=1,2 do
	...
	dropDown.MaxLines = 0
		...
		dropDown.Slider:SetFrameLevel((dropDown:GetFrameLevel() or 0) + 10)
	dropDown.Slider:SetScript("OnValueChanged",function (self,value)
		value = Round(value)
		self:GetParent().Position = value
		ELib.ScrollDropDown:Reload()
...
do
	...
	function ELib.ScrollDropDown.CreateButton(i,level,explicitDropDown)
		level = level or 1
		local dropDown = explicitDropDown or ELib.ScrollDropDown.DropDownList[level]
		...
		if dropDown.isModern then
			...
			button.radioButton = CreateFrame("CheckButton",nil,button)
		end
...
function ELib.ScrollDropDown.ClickButton(self)
	if ELib.ScrollDropDown.IsOpen() then
		ELib:DropDownClose()
		return
	end
	local dropDown = nil
...
function ELib.ScrollDropDown:Reload(level)
	for j=1,#ELib.ScrollDropDown.DropDownList do
		local _d = ELib.ScrollDropDown.DropDownList[j]
		local _shown = _d._logicalShown
		if _shown == nil then _shown = _d:IsShown() end
		if _shown or level == j then
			...
			for i=val,count do
				...
				if not data.isHidden then
					...
					if data.icon or data.atlas then
						...
						local _skipIcon = _dbg and _dbg.SkipBossIcons
						if _skipIcon then
							icon:SetTexture(nil)
...
function ELib.ScrollDropDown.Update(self, elapsed)
	...
	elseif ( self.showTimer < 0 ) then
		self.showTimer = nil
		self.isCounting = nil
		if ELib.ScrollDropDown.Close then
...
function ELib.ScrollDropDown.ToggleDropDownMenu(self,level,customList,customWidth)
	...
	local _prof = (VMRT and VMRT.DebugReminder and VMRT.DebugReminder.ProfileFreeze) or nil
	local _profThresh = (type(_prof) == "table" and _prof.thresholdMs)
		or (VMRT and VMRT.DebugReminder and VMRT.DebugReminder.thresholdMs)
		or 5
	local _profMarks = _prof and {} or nil
	local _profStart = _prof and debugprofilestop and debugprofilestop
	local function _profBegin(label)
		if not _profMarks then return end
		_profMarks[label] = _profStart()
	end
	local function _profEnd(label)
		if not _profMarks then return end
		local s = _profMarks[label]
		if not s then return end
		_profMarks[label] = nil
		local elapsed = _profStart() - s
		if elapsed > _profThresh then
			print(string.format(
				"|cffffaa00[DropDown]|r [L%d] %s = %.1fms",
				level or 0, label, elapsed
			))
		end
	end
	...
	local DROPDOWN_DEFAULT_LINES_CAP = 30

	_profBegin("count+cap")
	local maxLinesNow
	if self.Lines then
		maxLinesNow = self.Lines
	else
		local cc = 0
	...
	if maxLinesNow > DROPDOWN_DEFAULT_LINES_CAP then
		maxLinesNow = DROPDOWN_DEFAULT_LINES_CAP
	end
	_profEnd("count+cap")

	local _cbCount = max(maxLinesNow - dropDown.MaxLines, 0)
	local _cbLabel = string.format("CreateButton x%d", _cbCount)
	_profBegin(_cbLabel)
	...
	_profEnd("SetSize loop")

	_profBegin("additionalToggle")
	dropDown.Position = 1
	...
	_profEnd("additionalToggle")

	_profBegin("layout+slider")
	...
	_profEnd("layout+slider")

	local _profSS = (VMRT and VMRT.DebugReminder and VMRT.DebugReminder.ProfileShowStrata) or nil
	local _profSSThresh = (type(_profSS) == "table" and _profSS.thresholdMs) or _profThresh
	local _ssFirst
	if _profSS then
		ELib.ScrollDropDown._showStrataSeen = ELib.ScrollDropDown._showStrataSeen or {}
		local seenKey = tostring(level) .. ":" .. tostring(dropDown)
		_ssFirst = not ELib.ScrollDropDown._showStrataSeen[seenKey]
		ELib.ScrollDropDown._showStrataSeen[seenKey] = true
	end
	local _profSSStart = _profSS and debugprofilestop and debugprofilestop
	local _ssMarks = _profSS and {}
	local function _ssBegin(label)
		if not _ssMarks then return end
		_ssMarks[label] = _profSSStart()
	end
	local function _ssEnd(label)
		if not _ssMarks then return end
		local s = _ssMarks[label]
		if not s then return end
		_ssMarks[label] = nil
		local elapsed = _profSSStart() - s
		if elapsed > _profSSThresh then
			print(string.format(
				"|cffff7799[show+strata]|r [L%d %s] %s = %.1fms",
				level or 0, _ssFirst and "first" or "reopen", label, elapsed
			))
		end
	end
	...
	local _dbg = VMRT and VMRT.DebugReminder
	local _skipReloadBeforeShow = _dbg and _dbg.SkipReloadBeforeShow
	if not _skipReloadBeforeShow then
		_profBegin("Reload(pre-show)")
		ELib.ScrollDropDown:Reload(level)
		_profEnd("Reload(pre-show)")
	end

	_profBegin("show+strata")
	_ssBegin("dropDown:Show")
...
function ELib.ScrollDropDown.Close()
	_logicalHide(ELib.ScrollDropDown.DropDownList[1])
...
do
	...
	local function isInDropDownTree(focus)
		if not focus then return false end
		local list = ELib.ScrollDropDown.DropDownList
		if not list then return false end
		local f = focus
		local guard = 0
		while f and guard < 64 do
			for i=1, #list do
				if f == list[i] then
					return true
				end
				local anchor = list[i] and list[i].parent
				if anchor and f == anchor then
					return true
				end
			end
	...
	watcher:SetScript("OnUpdate", function(self)
		if not ELib.ScrollDropDown.IsOpen() then
			wasDown = false
			return
		end
		...
		if isDown and not wasDown then
			...
			if not isInDropDownTree(focus) then
				ELib.ScrollDropDown.Close()
			end
...
do
	...
	local function GraphGetNode(self,i)
		if not self.graph[i] then
			...
			SetSolidColor(self.graph[i],0.6, 1, 0.6, 1)
		end
		return self.graph[i]
	end
	...
	local function GraphSetVLine(self,i,X)
		if not self.vlines[i] then
			...
			SetSolidColor(self.vlines[i],1, 0.6, 0.6, .5)
	...
	local function GraphReload(self)
		...
		if not result then
			result = GraphReload_BuildNodes(self,minX,minY,maxX,maxY,0.04)
		end
		if not result then
			result = GraphReload_BuildNodes(self,minX,minY,maxX,maxY,0.08,true)
		end
		if not result then
		...
		if IsInFocus(self,x,y) then
			if #self.tooltipsData == 1 then
				...
				if Y then
					...
					GameTooltip:AddLine((main.name and main.name..": " or "")..(yText or Round(Y)))
					if comment then
	...
	local function GraphCreateZoom(self)
		...
		SetSolidColor(self.selectingTexture,0, 0.65, 0.9, .7)
	...
	local function GraphAddOordLines(self,num)
		...
		for i=1,num do
			...
			if not lineFrame then
				...
				SetSolidColor(lineFrame.line,0.6, 0.6, 1, 1)
	...
	function ELib.CreateGraph(parent,width,height,relativePoint,x,y,enableZoom)
		...
		SetSolidColor(self.axisX,0.6, 0.6, 1, 1)
		...
		SetSolidColor(self.axisY,0.6, 0.6, 1, 1)
...
do
	...
	local function Widget_Hyperlinks(self)
		...
		if eb.SetHyperlinksEnabled then
			pcall(eb.SetHyperlinksEnabled, eb, true)
		end
		if eb.HasScript and eb.SetScript then
			if eb:HasScript("OnHyperlinkEnter") then
				pcall(eb.SetScript, eb, "OnHyperlinkEnter", ELib.Tooltip.Edit_Show)
			end
			if eb:HasScript("OnHyperlinkLeave") then
				pcall(eb.SetScript, eb, "OnHyperlinkLeave", ELib.Tooltip.Hide)
			end
			if eb:HasScript("OnHyperlinkClick") then
				pcall(eb.SetScript, eb, "OnHyperlinkClick", ELib.Tooltip.Edit_Click)
			end
		end
		return self
	end
	...
	do
		local LUA_COLOR1 = "f92672"
		local LUA_COLOR2 = "e6db74"
		local LUA_COLOR3 = "75715e"
		local LUA_COLOR4 = "5dffed"
		local LUA_COLOR5 = "ae81ff"
	...
	function ELib:MultiEdit(parent)
		...
		self.EditBox:Point("BOTTOMLEFT",self.C,0,0):Point("BOTTOMRIGHT",self.C,0,0)

		local _origSize = self.Size
		self.Size = function(self_, w, h)
			local ret = _origSize(self_, w, h)
			if self_.content then
				if h then self_.content:SetHeight(h) end
				if self_.SetScrollChild then
					pcall(self_.SetScrollChild, self_, self_.content)
				end
			end
			if self_.EditBox then
				if w then self_.EditBox:SetWidth(w) end
				if h then self_.EditBox:SetHeight(h) end
			end
			return ret
		end
		...
		if self.HookScript then
			self:HookScript("OnSizeChanged",function(f,w,h)
				if f.content then
					f.content:SetHeight(h or 0)
					if f.SetScrollChild then
						pcall(f.SetScrollChild, f, f.content)
					end
				end
				if f.EditBox then
					f.EditBox:SetWidth(w)
					f.EditBox:SetHeight(h)
				end
			end)

			self:HookScript("OnShow", function(f)
				local sw, sh = f:GetWidth(), f:GetHeight()
				if f.content and sh and sh > 0 then
					f.content:SetHeight(sh)
					if f.SetScrollChild then
						pcall(f.SetScrollChild, f, f.content)
					end
				end
				if f.EditBox then
					if sw and sw > 0 then f.EditBox:SetWidth(sw) end
					if sh and sh > 0 then f.EditBox:SetHeight(sh) end
				end
				if f.SetVerticalScroll then
					pcall(f.SetVerticalScroll, f, 0)
				end
				if f.ScrollBar and f.ScrollBar.SetValue then
					pcall(f.ScrollBar.SetValue, f.ScrollBar, 0)
				end
			end)
		end
		...
		if self.EditBox.SetTextColor then self.EditBox:SetTextColor(1,1,1,1) end
		if self.EditBox.SetCursorColor then self.EditBox:SetCursorColor(1,1,1,1) end
		if self.EditBox.SetFont then
			local _src = ExRTFontNormal or GameFontNormal
			if _src and _src.GetFont then
				local _fp, _fs, _ff = _src:GetFont()
				if _fp then self.EditBox:SetFont(_fp, _fs or 12, _ff) end
			end
		end
	...
	function ELib.CreateMultilineEditBox(parent,width,height,relativePoint,x,y)
...
do
	local function Widget_Texture(self,texture,layer,cB,cA,altLayer)
		...
		if type(texture)=='number' and type(layer)=='number' then
			SetSolidColor(self.texture,texture,layer,cB,cA)
		else
...
do
	local function SetBorderColor(self,colorR,colorG,colorB,colorA,layerCounter)
		...
		SetSolidColor(self["border_top"..layerCounter],colorR,colorG,colorB,colorA)
		SetSolidColor(self["border_bottom"..layerCounter],colorR,colorG,colorB,colorA)
		SetSolidColor(self["border_left"..layerCounter],colorR,colorG,colorB,colorA)
		SetSolidColor(self["border_right"..layerCounter],colorR,colorG,colorB,colorA)
	end
	function ELib:Border(parent,size,colorR,colorG,colorB,colorA,outside,layerCounter)
		...
		SetSolidColor(top,colorR,colorG,colorB,colorA)
		SetSolidColor(bottom,colorR,colorG,colorB,colorA)
		SetSolidColor(left,colorR,colorG,colorB,colorA)
		SetSolidColor(right,colorR,colorG,colorB,colorA)
	...
	local function Widget_Update(self)
		...
		for i=1,maxX do
			...
			for j=1,#d do
				...
				SetSolidColor(media,unpack(color))
				media:Show()
	...
	local function Widget_OnUpdate(self)
		...
		if tip then
			if self.tip_IsShown ~= tip then
				...
				GameTooltip:SetPoint("BOTTOMLEFT", tip, "TOPRIGHT", 4, 0)
...
do
	...
	local function GetButton(self,level,uid)
		...
		SetSolidColor(button.sub.back,.2,.2,.2,.9)
	...
	local function UpdateT1Button(self,now,isUpdateReq,topStart,BUTTON_HEIGHT,GROUP_HEIGHT,buttonWidth,parent)
		...
		if parent then
			...
			SetSolidColor(button.sub.back,.3,.3,.3,.9)
		else
