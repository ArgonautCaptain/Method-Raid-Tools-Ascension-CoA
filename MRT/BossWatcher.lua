...
module.db.nowNum = 1
local fightData,guidData,graphData,reactionData,segmentsData,classData = nil
...
module.db.schoolsColors = {
	...
	[SCHOOL_MASK_NATURE or Enum.Damageclass.MaskNature] 	= {r=.6,g=1,b=.4},
...
module.db.schoolsNames = {
	...
	[0x1C] = L.BossWatcherSchoolElemental,
	[0x7C] = L.BossWatcherSchoolChromatic,
	[0x7E] = L.BossWatcherSchoolMagic,
	[0x7F] = L.BossWatcherSchoolChaos,
}
...
module.db.reductionAuras = {
	...
	[22812] = {0.8,nil,function(_,auraVar) return (100+auraVar)/100 end},
	[102342] = {0.8,nil,function(auraVar) return (100+auraVar)/100 end},


	[212800] = {0.65,nil,function(_,_,auraVar) return (100+auraVar)/100 end},
	...
	[104773] = {0.6,nil,function(_,_,auraVar) return (100+auraVar)/100 end},


	[48792] = {0.7,nil,function(_,_,auraVar) return (100+auraVar)/100 end},


	[122278] = 0.75,
	[120954] = {0.8,nil,function(_,auraVar) return (100+auraVar)/100 end},
	...
	[303350] = 0.9,
	[296003] = {1,nil,function(_,auraVar) return (100+auraVar)/100 end},
}
module.db.reductionBySpec = {
	[63] = {30482,	0.94,	ReductionAurasFunctions.physical,	0x4},
	[104] = {16931,	0.94},
	[581] = {203513,0.9},
}
...
local _bw_tracked_overflow = false
local _bw_graph_count = 0
local _bw_tracked_count = 0
local _bw_seg_interval = 1
local function _bw_estimateFightKB(fd)
	if type(fd) ~= "table" then return 0 end
	local kb = 1
	if fd.auras then kb = kb + #fd.auras * 0.45 end
	if fd.tracking then kb = kb + #fd.tracking * 0.10 end
	if fd.dies then kb = kb + #fd.dies * 0.10 end
	if fd.summons then kb = kb + #fd.summons * 0.10 end
	if fd.chat then kb = kb + #fd.chat * 0.20 end
	if fd.dispels then kb = kb + #fd.dispels * 0.10 end
	if fd.aurabroken then kb = kb + #fd.aurabroken * 0.10 end
	if fd.interrupts then kb = kb + #fd.interrupts * 0.10 end
	if fd.resurrests then kb = kb + #fd.resurrests * 0.10 end
	if fd.cast then
		for _, t in pairs(fd.cast) do kb = kb + (#t * 0.10) + 0.05 end
	end
	if fd.graphData then
		local n = 0
		for _ in pairs(fd.graphData) do n = n + 1 end
		kb = kb + n * 1.50
	end
	if fd.damage then
		for _, dt in pairs(fd.damage) do
			for _, df in pairs(dt) do
				for _, ds in pairs(df) do
					for _, dsp in pairs(ds) do
						for _ in pairs(dsp) do kb = kb + 0.25 end
					end
				end
			end
		end
	...
	if fd.heal then
		for _, ht in pairs(fd.heal) do
			for _, hd in pairs(ht) do
				for _, hf in pairs(hd) do
					for _, hsp in pairs(hf) do
						for _ in pairs(hsp) do kb = kb + 0.25 end
					end
				end
			end
		end
...
function module.options:Load()
	...
	self.optionNoBuffs = ELib:Check(
		self,
		L.BossWatcherOptionNoBuffs or "Disable buffs & debuffs",
		VMRT.BossWatcher.optionNoBuffs
	):Point(15,-180):Tooltip(L.BossWatcherOptionNoBuffsTooltip or
		"Buffs and debuffs - the main consumers of large amounts of memory"):
	OnClick(function(self)
		if self:GetChecked() then
			VMRT.BossWatcher.optionNoBuffs = true
		else
	...
	self.optionLessSegments = ELib:Check(
		self,
		(L.BossWatcherOptionImproved and ("|cffff8080"..(L.BossWatcherOptionImproved):gsub("|c%x%x%x%x%x%x%x%x",""):gsub("|r","").." (off)")) or "Reduced detail (5s segments)",
		VMRT.BossWatcher.optionLessSegments
	):Point(260,-180):Tooltip((L.BossWatcherOptionImprovedTooltip or
		"New segments automatically begins every second.\nTurning this ON drops to one segment every 5 seconds.") .. "\n|cff80ff80Off-by-default. Enable on long fights to cut memory usage ~5x.|r"):
	OnClick(function(self)
		if self:GetChecked() then
			VMRT.BossWatcher.optionLessSegments = true
		else
	...
	self.saveVariables = ELib:Check(self,L.BossWatcherSaveVariables,VMRT.BossWatcher.saveFightsPersist):Point(260,-35):Tooltip(L.BossWatcherSaveVariablesWarring):OnClick(function(self)
		if self:GetChecked() then
			...
			print(L.BossWatcherSaveVariablesSaved)
		else
...
function module.main:ADDON_LOADED()
	...
	if (VMRT.BossWatcher.saveFightsPersist or VMRT.BossWatcher.saveVariables) and VMRT.BossWatcher.SAVED_DATA then
	...
	elseif not VMRT.BossWatcher.saveFightsPersist then
	...
	module:RegisterEvents('PLAYER_LOGOUT')
end

function module.main:PLAYER_LOGOUT()
	if VMRT.BossWatcher.saveFightsPersist then
		VMRT.BossWatcher.SAVED_DATA = module.db.data
	end
end
...
SLTReductionFrame:SetScript("OnEvent",function(_,_,unit)
	...
	elseif findEm and not SLTReductionAuraData[ guid ] then
		...
		if not VMRT.BossWatcher.optionNoBuffs then
			local _n = #fightData_auras
			if _n < BW_AURA_CAP then
				fightData_auras[_n + 1] = {GetTime() - module.db.timeFix[1] + module.db.timeFix[2],SLTReductionSourceGUID,UnitGUID(unit),true,true,SLTReductionAuraSpellID,"BUFF",1,1}
			else
				_bw_aura_overflow = true
			end
		end
	elseif not findEm and SLTReductionAuraData[ guid ] then
		...
		if not VMRT.BossWatcher.optionNoBuffs then
			local _n = #fightData_auras
			if _n < BW_AURA_CAP then
				fightData_auras[_n + 1] = {GetTime() - module.db.timeFix[1] + module.db.timeFix[2],SLTReductionSourceGUID,UnitGUID(unit),true,true,SLTReductionAuraSpellID,"BUFF",2,1}
			else
				_bw_aura_overflow = true
			end
		end
...
function _BW_Start(encounterID,encounterName)
	...
	_bw_seg_interval = (VMRT.BossWatcher.optionLessSegments and 5) or 1
	_bw_aura_overflow = false
	_bw_cast_overflow = false
	_bw_graph_overflow = false
	_bw_tracked_overflow = false
	_bw_graph_count = 0
	_bw_tracked_count = 0
	...
	trackedGUIDs = fightData.trackedguids
	guidData = fightData.guids
	classData = fightData.classes
	...
	active_segment = 1
	active_phase = 1
	if not segmentsData[1] then
		segmentsData[1] = { e = true, t = GetTime(), p = active_phase }
	end
	...
	local _selfGUID = UnitGUID and UnitGUID("player")
	if _selfGUID then
		trackedGUIDs[_selfGUID] = true
		raidGUIDs[_selfGUID] = raidGUIDs[_selfGUID] or (UnitName and UnitName("player")) or true
	end
	local petUnits = {"pet","partypet1","partypet2","partypet3","partypet4"}
	for i=1,40 do petUnits[#petUnits+1] = "raidpet"..i end
	for i=1,#petUnits do
		local petGUID = UnitGUID and UnitGUID(petUnits[i])
		if petGUID then
			trackedGUIDs[petGUID] = true
		end
	end
end
...
function _BW_End(encounterID)
	if fightData then
		...
		if encounterID and not fightData.encounterID then
			fightData.encounterID = encounterID
		end
	...
	for i=25,1,-1 do
		...
		if data and data.encounterStart and data.encounterEnd and data.encounterID then
			local hasRaid = false
			if data.raidguids and data.guids then
				for guid,_ in pairs(data.guids) do
					if data.raidguids[guid] or guid == _selfGUID then
						hasRaid = true
						break
					end
				end
	...
	local _bw_evicted_for_budget = 0
	local _bw_oversize_warn = false
	do
		local sizes = {}
		local total_kb = 0
		for i=1,#module.db.data do
			sizes[i] = _bw_estimateFightKB(module.db.data[i])
			total_kb = total_kb + sizes[i]
		end
		if total_kb > BW_TOTAL_BUDGET_KB then
			for i=#module.db.data,2,-1 do
				module.db.data[i] = nil
				total_kb = total_kb - (sizes[i] or 0)
				_bw_evicted_for_budget = _bw_evicted_for_budget + 1
				if total_kb <= BW_TOTAL_BUDGET_KB then break end
			end
			if total_kb > BW_TOTAL_BUDGET_KB and sizes[1] and sizes[1] > BW_TOTAL_BUDGET_KB then
				_bw_oversize_warn = sizes[1]
			end
		end
	end
	...
	if _bw_aura_overflow or _bw_cast_overflow or _bw_graph_overflow or _bw_tracked_overflow then
		local hits = {}
		if _bw_aura_overflow then hits[#hits+1] = "auras" end
		if _bw_cast_overflow then hits[#hits+1] = "casts" end
		if _bw_graph_overflow then hits[#hits+1] = "graph" end
		if _bw_tracked_overflow then hits[#hits+1] = "trackedGUIDs" end
		print(string.format(
			"|cffff8080[MRT/BossWatcher]|r recorded fight log truncated (%s cap reached). Consider enabling 'Disable buffs & debuffs' or 'Reduced detail (5s segments)' on long fights to avoid the cap.",
			table.concat(hits, "+")
		))
		_bw_aura_overflow = false
		_bw_cast_overflow = false
	...
	if _bw_evicted_for_budget > 0 then
		print(string.format(
			"|cffff8080[MRT/BossWatcher]|r evicted %d old saved fight(s) to stay under the %d MB cumulative memory budget. Lower the 'Saved fights' slider or enable 'Disable buffs & debuffs' to keep more history.",
			_bw_evicted_for_budget,
			BW_TOTAL_BUDGET_KB / 1024
		))
	end
	if _bw_oversize_warn then
		print(string.format(
			"|cffff8080[MRT/BossWatcher]|r last fight estimate %.0f MB exceeds the %d MB budget on its own. Future pulls of this length will likely OOM the client; enable 'Disable buffs & debuffs' or 'Reduced detail (5s segments)'.",
			_bw_oversize_warn / 1024,
			BW_TOTAL_BUDGET_KB / 1024
		))
	end
...
do
	...
	function module:timer(elapsed)
		...
		local _interval = _bw_seg_interval
		local nowTimer = _graphSectionTimer - _graphSectionTimer % _interval + _interval
		if _graphSectionTimerRounded ~= nowTimer then
			_graphSectionTimerRounded = nowTimer
			if _bw_graph_count < BW_GRAPH_CAP then
			_bw_graph_count = _bw_graph_count + 1
			local data = {}
			...
			for i=1,#_graphRaidSnapshot do
				...
				_bw_graph_overflow = true
			end
		end

		segment_tmr = segment_tmr + elapsed
		local nowSegment = segment_tmr - segment_tmr % _interval
...
local function GetCurrentZoneID()
	...
	elseif zoneType == 'raid' and ((tonumber(mapID) and mapID >= 603) or mapID == 533) then
		zoneID = 1
	elseif ExRT.isWotLKOnly and zoneType == 'raid' then
		zoneID = 1
	elseif zoneType == 'arena' or zoneType == 'pvp' then
		zoneID = 2
	elseif zoneType == 'scenario' and (difficulty == 38 or difficulty == 39 or difficulty == 40 or difficulty == 45) then
		zoneID = 4
	elseif mapID == 2274 or mapID == 2212 or mapID == 2213 or mapID == 2275 then
		zoneID = 4
...
function module.main:ENCOUNTER_START(encounterID,encounterName)
	...
	if zoneID == 1 then
		if fightData then
			if not fightData.encounterID and encounterID then
				fightData.encounterID = encounterID
			end
			if not fightData.encounterName and encounterName and encounterName ~= "" then
				fightData.encounterName = encounterName
			end
		else
...
function module.main:PLAYER_REGEN_DISABLED()
	...
	if zoneID == 0 then
		_BW_Start()
	elseif ExRT.isWotLKOnly and zoneID == 1 and not fightData then
		_BW_Start()
	end
end
...
function module.main:PLAYER_REGEN_ENABLED()
	...
	if zoneID == 0 then
		_BW_End()
	elseif ExRT.isWotLKOnly and zoneID == 1 and fightData then
		_BW_End()
	end
end
...
function module.main.SPELL_DAMAGE(timestamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,val1,val2,val3,val4,val5,val6,val7,val8,val9,val10,val11,val12,val13,val14)
	...
	local destReaction = destTable[destFlags]
	...
	local sourceTable = destReaction[sourceGUID]
	...
	local segmentsTable = sourceTable[event == "SPELL_PERIODIC_DAMAGE" and -val1 or val1]
	...
	do
		...
		activeTable.mt = (type(val14) == "string") and val14 or nil
	...
	if reductionTable then
		...
		for i=1,#reductionTable do
			...
			elseif isCheck == 3 then
				isCheck = module.db.reductionIsNotAoe[val1]
			elseif isCheck == 4 then
...
function module.main.SPELL_HEAL(timestamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,val1,val2,val3,val4,val5,val6,val7)
	...
	local segmentsTable = destReactTable[event == "SPELL_PERIODIC_HEAL" and -val1 or val1]
	if not segmentsTable then
		segmentsTable = {
			[active_segment] = {
				amount = 0,
	...
	elseif val1 == 186263 then
		spellFix_SM[destGUID] = spellTable
	elseif val1 == 213313 then
...
function module.main.SPELL_AURA_APPLIED(timestamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellID,spellName,school,auraType)
	...
	if not VMRT.BossWatcher.optionNoBuffs then
		local _n = #fightData_auras
		if _n < BW_AURA_CAP then
			fightData_auras[_n + 1] = {timestamp,sourceGUID,destGUID,bit_band(sourceFlags or 0,240) == 16,bit_band(destFlags or 0,240) == 16,spellID,auraType,1,1,s = active_segment}
		else
			_bw_aura_overflow = true
		end
	end
	...
	if spellID == 45181 or spellID == 211319 or spellID == 87024 or spellID == 344907 or spellID == 116888 or spellID == 209261 then
...
function module.main.SPELL_AURA_REMOVED(timestamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellID,_,school,auraType,amount)
	...
	if not VMRT.BossWatcher.optionNoBuffs then
		local _n = #fightData_auras
		if _n < BW_AURA_CAP then
			fightData_auras[_n + 1] = {timestamp,sourceGUID,destGUID,bit_band(sourceFlags or 0,240) == 16,bit_band(destFlags or 0,240) == 16,spellID,auraType,2,1,s = active_segment}
		else
			_bw_aura_overflow = true
		end
	end
...
function module.main.SPELL_CAST_SUCCESS(timestamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellID,spellName,spellSchool,empoweredRank)
	...
	do
		local _n = #sourceTable
		if _n < BW_CAST_CAP_PER_SOURCE then
			sourceTable[_n + 1] = {timestamp,spellID,1,destGUID,destFlags2,sourceFlags2,sourceFlags,empoweredRank,s = active_segment}
		else
			_bw_cast_overflow = true
		end
	end
...
function module.main.SPELL_AURA_APPLIED_DOSE(timestamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellID,_,_,type,stack)
	...
	if not VMRT.BossWatcher.optionNoBuffs then
		local _n = #fightData_auras
		if _n < BW_AURA_CAP then
			fightData_auras[_n + 1] = {timestamp,sourceGUID,destGUID,bit_band(sourceFlags or 0,240) == 16,bit_band(destFlags or 0,240) == 16,spellID,type,3,stack,s = active_segment}
		else
			_bw_aura_overflow = true
		end
	end
...
function module.main.SPELL_CAST_START(timestamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellID,spellName,spellSchool)
	...
	do
		local _n = #sourceTable
		if _n < BW_CAST_CAP_PER_SOURCE then
			sourceTable[_n + 1] = {timestamp,spellID,2,destGUID,destFlags2,sourceFlags2,sourceFlags,s = active_segment}
		else
			_bw_cast_overflow = true
		end
	end
...
function module.main.SPELL_MISSED(timestamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellID,_,school,missType,isOffHand,amountMissed,critical)
	...
	if missType == "ABSORB" then
		return CLEU[newEvent](timestamp,newEvent,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellID,nil,nil,0,0,school,nil,nil,amountMissed,critical,nil,nil,isOffHand,missType)
	elseif missType == "BLOCK" then
		return CLEU[newEvent](timestamp,newEvent,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellID,nil,nil,0,0,school,nil,amountMissed,nil,critical,nil,nil,isOffHand,missType)
...
function module.main.SPELL_AURA_REMOVED_DOSE(timestamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellID,_,_,type,stack)
	if not VMRT.BossWatcher.optionNoBuffs then
		local _n = #fightData_auras
		if _n < BW_AURA_CAP then
			fightData_auras[_n + 1] = {timestamp,sourceGUID,destGUID,bit_band(sourceFlags or 0,240) == 16,bit_band(destFlags or 0,240) == 16,spellID,type,4,stack,s = active_segment}
		else
			_bw_aura_overflow = true
		end
	end
...
local function _captureClass(guid, flags)
	if not guid or guid == "" or not classData or classData[guid] then return end
	if ExRT.F.GetUnitTypeByGUID(guid) ~= 0 then
		return
	end
	local ok, _, class = pcall(GetPlayerInfoByGUID, guid)
	if ok and class and ExRT.F.classSquareIcon and ExRT.F.classSquareIcon[class] then
		classData[guid] = class
	end
end
...
CLEUParser = function(timestamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,val1,val2,val3,val4,val5,val6,val7,val8,val9,val10,val11,val12,val13)
	...
	if trackedGUIDs then
		local sT = sourceGUID and trackedGUIDs[sourceGUID]
		local dT = destGUID and trackedGUIDs[destGUID]
		if not sT and not dT then
			return
		end
	...
	_captureClass(sourceGUID, sourceFlags)
	_captureClass(destGUID, destFlags)
...
function BWInterfaceFrameLoad()
	...
	local function GetPlayerInfoByGUID(guid)
		if type(guid) ~= "string" or guid == "" or guid:match("^0x?0+$") then
			return nil
		end
		local ok, a, b, c, d, e, f, g = pcall(_GetPlayerInfoByGUID, guid)
		if not ok then return nil end
		return a, b, c, d, e, f, g
	end
	local _GetSpellInfo = GetSpellInfo
	local _GetSpellInfoSafe = ExRT.F.GetSpellInfoSafe
	local _GetSpellTextureSafe = ExRT.F.GetSpellTextureSafe
	local function _resolveTex(spellID, name, rank, tex)
		if tex then return name, rank, tex end
		if _GetSpellTextureSafe then
			local t2 = _GetSpellTextureSafe(spellID)
			if t2 then return name, rank, t2 end
		end
		return name, rank, "Interface\\Icons\\INV_Misc_QuestionMark"
	end
	local GetSpellInfo = function(spellID)
		if not spellID then return nil end
		if type(spellID) == 'string' then
			local n, r, t = _GetSpellInfo(spellID)
			if n then return _resolveTex(spellID, n, r, t) end
			return spellID, false, "Interface\\Icons\\INV_Misc_QuestionMark"
		end
		if _GetSpellInfoSafe then
			local n, r, t = _GetSpellInfoSafe(spellID)
			if n then return _resolveTex(spellID, n, r, t) end
		end
		local n, r, t = _GetSpellInfo(spellID)
		if n then return _resolveTex(spellID, n, r, t) end
		local fallbackTex = (_GetSpellTextureSafe and _GetSpellTextureSafe(spellID)) or "Interface\\Icons\\INV_Misc_QuestionMark"
		return "spell:"..tostring(spellID), false, fallbackTex
	end
	...
	do
		...
		GetSpellTexture = function(spellID)
			local tex = _GetSpellTexture and _GetSpellTexture(spellID)
			if tex then return tex end
			if _GetSpellTextureSafe then return _GetSpellTextureSafe(spellID) end
			return nil
		end
	end
	...
	BWInterfaceFrame:Hide()
	if ExRT.F.AddonScaleApply then ExRT.F.AddonScaleApply(BWInterfaceFrame) end
	BWInterfaceFrame:HookScript("OnShow", function(self)
		local s = (VMRT and VMRT.Addon and tonumber(VMRT.Addon.Scale)) or 1
	...
	local function DamageTab_UpdateLinesPlayers()
		...
		for i=1,#damage do
			...
			if damageLine.guid and damageLine.guid ~= "" then
				...
				if not class and CurrentFight and CurrentFight.classes then
					class = CurrentFight.classes[damageLine.guid]
				end
			end
			local icon = ""
			if class then
				local cp,cl,cr,ct,cb = ExRT.F.classIconRaw(class)
				if cp then icon = {cp, cl, cr, ct, cb} end
			end
	...
	local function DamageTab_UpdateLinesSpellToTargets()
							...
							if sourceVar[spellID] or sourceVar[-spellID] then
								...
								for segment,spellAmount in pairs(spellSegments) do
									if CurrentFight.segments[segment].e then
										...
										damage[inDamagePos][3] = damage[inDamagePos][3] + spellAmount.count
										damage[inDamagePos][4] = damage[inDamagePos][4] + spellAmount.overkill
										damage[inDamagePos][5] = damage[inDamagePos][5] + spellAmount.blocked
										damage[inDamagePos][6] = damage[inDamagePos][6] + spellAmount.absorbed
										damage[inDamagePos][7] = damage[inDamagePos][7] + spellAmount.crit
										damage[inDamagePos][8] = damage[inDamagePos][8] + spellAmount.critcount
										damage[inDamagePos][9] = max(damage[inDamagePos][9],spellAmount.critmax)
										damage[inDamagePos][13] = max(damage[inDamagePos][13],spellAmount.hitmax)
										damage[inDamagePos][14] = damage[inDamagePos][14] + spellAmount.parry
										damage[inDamagePos][15] = damage[inDamagePos][15] + spellAmount.dodge
										damage[inDamagePos][16] = damage[inDamagePos][16] + spellAmount.miss
										total = total + spellAmount.amount - spellAmount.overkill
										totalOver = totalOver + spellAmount.overkill + spellAmount.blocked + spellAmount.absorbed
		...
		for i=1,#damage do
			...
			if damage[i][1] and damage[i][1] ~= "" then
				...
				if not class and CurrentFight and CurrentFight.classes then
					class = CurrentFight.classes[damage[i][1]]
				end
			end
			local icon = ""
			if class then
	...
	local function DamageTab_DPS(disableUpdateVars)
		...
		if not CurrentFight.damage then
			return
		end
	...
	local AurasTab_Variables = {
		...
		buffsFilters = {
			[1] = {[-1]=L.BossWatcherFilterOnlyBuffs,},
			[2] = {[-1]=L.BossWatcherFilterOnlyDebuffs,},
			[3] = {[-1]=L.BossWatcherFilterBySpellID,},
			[4] = {[-1]=L.BossWatcherFilterBySpellName,},
			[5] = {
	...
	function tab.targetsList:SetListValue(index)
		...
		local function SwitchSourceAccepted(sourceGUID)
			local t = ExRT.F.GetUnitTypeByGUID(sourceGUID)
			return t == 0 or t == 4 or t == nil
		end
		...
		if CurrentFight.switch[destGUID] then
			...
			if CurrentFight.switch[destGUID][1] then
				for sourceGUID,sourceData in pairs(CurrentFight.switch[destGUID][1]) do
					if SwitchSourceAccepted(sourceGUID) and not ExRT.F.table_find(switchTable,sourceGUID,3) then
	...
	tab:SetScript("OnShow",function (self)
		...
		if BWInterfaceFrame.nowFightID ~= self.lastFightID then
			...
			UpdateMobsPage()
			self.lastFightID = BWInterfaceFrame.nowFightID
			if self.targetsList.GUIDs and self.targetsList.GUIDs[1] then
				self.targetsList.selected = 1
				self.targetsList:Update()
				self.targetsList:SetListValue(1)
			end
		end
	end)
	tab:SetScript("OnHide",function (self)
		...
		elseif SpellsTab_Variables.Type ~= 3 then
			...
			if selfGUID then
				for i,PlayerCastData in ipairs(CurrentFight.cast[selfGUID]) do
					if (CurrentFight.segments[ PlayerCastData.s ] == nil or CurrentFight.segments[ PlayerCastData.s ].e) then
				...
				local relevantGUIDs = tab5._spellsRelevantGUIDs
				for GUID,dataGUID in pairs(CurrentFight.cast) do
					if not relevantGUIDs or relevantGUIDs[GUID] then
						local skipForeignPlayer = SpellsTab_isFriendly and GUID and GUID:find("^Player%-") and not SpellsTab_IsOurGroupPlayer(GUID)
						if not skipForeignPlayer then
							for i,PlayerCastData in ipairs(dataGUID) do
								if ExRT.F.GetUnitInfoByUnitFlag(PlayerCastData[7],2) == reaction and (CurrentFight.segments[ PlayerCastData.s ] == nil or CurrentFight.segments[ PlayerCastData.s ].e) then
			...
			for GUID,dataGUID in pairs(CurrentFight.cast) do
				if not selfGUID or selfGUID == GUID then
					for i,PlayerCastData in ipairs(CurrentFight.cast[GUID]) do
						if PlayerCastData[3] ~= 2 and (CurrentFight.segments[ PlayerCastData.s ] == nil or CurrentFight.segments[ PlayerCastData.s ].e) then
	...
	local function SpellsTab_IsOurGroupPlayer(guid)
		if not guid or guid == "" then return false end
		if CurrentFight.raidguids and CurrentFight.raidguids[guid] then return true end
		local pets = CurrentFight.pets
		if pets and ExRT.F.Pets and ExRT.F.Pets.getOwnerGUID then
			local owner = ExRT.F.Pets:getOwnerGUID(guid, pets)
			if owner and CurrentFight.raidguids and CurrentFight.raidguids[owner] then
				return true
			end
		end
		return false
	end
	local function SpellsTab_BuildOurGroupSet()
		local set = {}
		local raidguids = CurrentFight and CurrentFight.raidguids
		if raidguids then
			for guid in pairs(raidguids) do set[guid] = true end
		end
		local pets = CurrentFight and CurrentFight.pets
		if pets and raidguids and ExRT.F.Pets and ExRT.F.Pets.getOwnerGUID then
			for petGUID in pairs(pets) do
				local owner = ExRT.F.Pets:getOwnerGUID(petGUID, pets)
				if owner and raidguids[owner] then
					set[petGUID] = true
				end
			end
		end
	...
	local function SpellsTab_BuildRelevantGUIDs(ourset)
		local relevant = {}
		for guid in pairs(ourset) do relevant[guid] = true end
		local cast = CurrentFight and CurrentFight.cast
		if cast then
			for src, list in pairs(cast) do
				local srcInGroup = ourset[src]
				for i = 1, #list do
					local castData = list[i]
					local dst = castData and castData[4]
					if srcInGroup then
						if dst and dst ~= "" then relevant[dst] = true end
					elseif dst and dst ~= "" and ourset[dst] then
						relevant[src] = true
					end
				end
			end
		end
		local damage = CurrentFight and CurrentFight.damage
		if damage then
			for dst, byDestFlags in pairs(damage) do
				for _, bySrc in pairs(byDestFlags) do
					for src in pairs(bySrc) do
						if ourset[src] and dst and dst ~= "" then
							relevant[dst] = true
						end
						if ourset[dst] and src and src ~= "" then
							relevant[src] = true
						end
					end
				end
		...
		local heal = CurrentFight and CurrentFight.heal
		if heal then
			for dst, byDestFlags in pairs(heal) do
				for _, bySrc in pairs(byDestFlags) do
					for src in pairs(bySrc) do
						if ourset[src] and dst and dst ~= "" then
							relevant[dst] = true
	...
	local function UpdateSpellsPage()
		...
		local ourset = SpellsTab_BuildOurGroupSet()
		local relevant = SpellsTab_BuildRelevantGUIDs(ourset)
		tab5._spellsRelevantGUIDs = relevant
		...
		if SpellsTab_Variables.Type ~= 4 then
			...
			for sourceGUID,sourceData in pairs(CurrentFight.cast) do
				if not ExRT.F.table_find(playersListTable,sourceGUID,1) then
					local skipForeign = false
					if not relevant[sourceGUID] then
						skipForeign = true
					elseif SpellsTab_isFriendly and sourceGUID:find("^Player%-") and not SpellsTab_IsOurGroupPlayer(sourceGUID) then
						skipForeign = true
					end
					if not skipForeign then
					for i=1,#sourceData do
						if
							(CurrentFight.segments[ sourceData[i].s ] == nil or CurrentFight.segments[ sourceData[i].s ].e) and
			...
			for _,data in pairs(CurrentFight.summons) do
				...
				if (CurrentFight.segments[ data.s ] == nil or CurrentFight.segments[ data.s ].e) and not ExRT.F.table_find(playersListTable,sourceGUID,1) then
	...
	local function SpellPage_TimeLine_Spell_OnEnter(self)
		...
		if not spellName then
			spellName = spellID and ("spell:"..tostring(spellID)) or "?"
		end
		spellTexture = spellTexture or "Interface\\Icons\\INV_Misc_QuestionMark"
		GameTooltip:SetOwner(self,"ANCHOR_LEFT")
		GameTooltip:SetText("|T"..spellTexture..":20|t "..spellName)
		local t = data.time
		local castTime
		if data.time_end or data.failed then
			...
			if data.interrupt then
				...
				local ispellName,_,ispellTexture = GetSpellInfo(data.interrupt_data[3] or 0)
				ispellName = ispellName or ("spell:"..tostring(data.interrupt_data[3] or "?"))
				ispellTexture = ispellTexture or "Interface\\Icons\\INV_Misc_QuestionMark"
				GameTooltip:AddLine(format("%s %s %s %s",
					...
					"|T"..ispellTexture..":16|t "..ispellName
				))
			end
		end
		...
		if GameTooltip.AddSpellByID then
			pcall(GameTooltip.AddSpellByID, GameTooltip, spellID)
		end
	...
	function tab.timelineFrame:UpdateDB()
		...
		for sourceGUID,sourceData in pairs(CurrentFight.cast) do
			local skipForeign = false
			if SpellsTab_isFriendly and sourceGUID and sourceGUID:find("^Player%-") and not SpellsTab_IsOurGroupPlayer(sourceGUID) then
				skipForeign = true
			end
			if not skipForeign and ExRT.F.GetUnitInfoByUnitFlag(CurrentFight.reaction[sourceGUID],2) == reaction then
				local guidData = {
					guid = sourceGUID,
	...
	function SpellsPage_GetCastsNumber(guidsSourceTable,guidsDestTable,spellID)
		...
		for GUID,dataGUID in pairs(CurrentFight.cast) do
			if not guidsSourceTable or guidsSourceTable[GUID] then
				for i,PlayerCastData in ipairs(dataGUID) do
					if allData and PlayerCastData[3] ~= 2 and (CurrentFight.segments[ PlayerCastData.s ] == nil or CurrentFight.segments[ PlayerCastData.s ].e) and (not guidsDestTable or not PlayerCastData[4] or guidsDestTable[ PlayerCastData[4] ]) then
						allData[ PlayerCastData[2] ] = (allData[ PlayerCastData[2] ] or 0) + 1
					elseif not allData and PlayerCastData[3] ~= 2 and (PlayerCastData[2] == spellID or (spellName and spellName == GetSpellInfo(PlayerCastData[2]))) and (CurrentFight.segments[ PlayerCastData.s ] == nil or CurrentFight.segments[ PlayerCastData.s ].e) then
						count = count + 1
					end
				end
			end
	...
	local function GraphTab_UpdatePage()
		...
		local defaultPower, bestCount = nil, -1
		for powerID in pairs(powers) do
			local cnt = (units[powerID] and #units[powerID]) or 0
			if cnt > bestCount or (cnt == bestCount and (defaultPower == nil or powerID < defaultPower)) then
				bestCount = cnt
				defaultPower = powerID
			end
		end
		local _, playerClass = UnitClass("player")
		local primaries = playerClass and energyPerClass[playerClass]
		if primaries and primaries[1] then
			for _, pid in ipairs(primaries[1]) do
				local cnt = (units[pid] and #units[pid]) or 0
				if cnt == bestCount and powers[pid] then
					defaultPower = pid
					break
				end
			end
		end
		GraphsTab_Variables.PowerTypeNow = defaultPower or 0
		GraphTab_ReloadPage()
	end
	...
	local function EnergyLineOnEnter(self)
		if self.spellID then
			GameTooltip:SetOwner(self, "ANCHOR_LEFT")
			local ok = pcall(GameTooltip.SetHyperlink, GameTooltip, "spell:"..self.spellID)
			local hasText = _G["GameTooltipTextLeft1"] and _G["GameTooltipTextLeft1"]:GetText()
			if not ok or not hasText or hasText == "" then
				local n = ExRT.F.GetSpellInfoSafe and ExRT.F.GetSpellInfoSafe(self.spellID)
				GameTooltip:ClearLines()
				if n then
					GameTooltip:AddLine(n)
				else
					GameTooltip:AddLine("Spell "..tostring(self.spellID))
				end
			end
			GameTooltip:Show()
		end
	...
	function tab.powerTypeList:SetListValue(index)
		...
		if #spellList > 1 then
			for i,spellData in ipairs(spellList) do
				...
				if line then
					line.texture:SetTexture(spellData[3] or "Interface\\Icons\\INV_Misc_QuestionMark")
					line.spellName:SetText(spellData[2] or ("Spell "..tostring(spellData[1])))
	...
	function tab.list:SetListValue(index)
		...
		if index == 2 then
			...
			for i,line in ipairs(workTable) do
				...
				local segE = CurrentFight.segments[ line.s ]
				segE = (segE == nil) or segE.e
				if toAdd and segE then
			...
			for i,line in ipairs(workTable) do
				...
				if isOkay then
					...
					local segE2 = CurrentFight.segments[ line.s ]
					segE2 = (segE2 == nil) or segE2.e
					if segE2 then
			...
			if isInterrupt and not filter then
				...
				for GUID,dataGUID in pairs(CurrentFight.cast) do
					for i,PlayerCastData in ipairs(dataGUID) do
						if (CurrentFight.segments[ PlayerCastData.s ] == nil or CurrentFight.segments[ PlayerCastData.s ].e) and SpellCanBeKicked[ PlayerCastData[2] ] and PlayerCastData[3] == 1 then
			...
			if isDispel and not filter then
				...
				for GUID,dataGUID in pairs(CurrentFight.cast) do
					for i,PlayerCastData in ipairs(dataGUID) do
						if (CurrentFight.segments[ PlayerCastData.s ] == nil or CurrentFight.segments[ PlayerCastData.s ].e) and SpellDispel[ PlayerCastData[2] ] and PlayerCastData[3] == 1 then
							local spellSourceName,_,spellSourceTexture = GetSpellInfo(PlayerCastData[2])
							local sourceMarker = module.db.raidTargets[ PlayerCastData[6] or 0 ]
							local destMarker = module.db.raidTargets[ PlayerCastData[5] or 0 ]
							local findEm = nil
	...
	function UpdateInterruptPage()
		...
		local data = {}
		for i,line in ipairs(workTable) do
			local seg = CurrentFight.segments[ line.s ]
			local segE = (seg == nil) or seg.e
			if Intterupt_Type == 1 then
				if not ExRT.F.table_find(data,line[1]) and segE then
					data[#data + 1] = line[1]
				end
			elseif Intterupt_Type == 2 then
				if not ExRT.F.table_find(data,line[2]) and segE then
					data[#data + 1] = line[2]
				end
			else
				if not ExRT.F.table_find(data,line[4]) and segE then
					data[#data + 1] = line[4]
				end
			end
		end
	...
	local function HealingTab_UpdateLinesPlayers()
									...
									for spellID,spellSegments in pairs(destReactData) do
										for segment,spellAmount in pairs(spellSegments) do
											if CurrentFight.segments[segment].e then
												...
												sourceHeal.total = sourceHeal.total + spellAmount.amount + spellAmount.absorbed
												sourceHeal.overheal = sourceHeal.overheal + spellAmount.over
												sourceHeal.absorbed = sourceHeal.absorbed + spellAmount.absorbed
												sourceHeal.crit = sourceHeal.crit + spellAmount.crit - (HealingTab_Variables.ShowOverheal and 0 or spellAmount.critover)
												sourceHeal.absorbs = sourceHeal.absorbs + spellAmount.absorbs
		...
		for i=1,#heal do
			...
			if healLine.guid and healLine.guid ~= "" then
				...
				if not class and CurrentFight and CurrentFight.classes then
					class = CurrentFight.classes[healLine.guid]
				end
			end
			local icon = ""
			if class then
							...
							if ExRT.F.table_len(HdestVar) == 0 or HdestVar[destGUID] then
								if (isEnemy and doEnemy) or (not isEnemy and not doEnemy) then
									for spellID,spellSegments in pairs(destReactData) do
										...
										for segment,spellAmount in pairs(spellSegments) do
											if CurrentFight.segments[segment].e then
												if spellID == 98021 and not HealingTab_Variables.ShowOverheal then
													spellAmount = HealingTab_Variables.NULLSpellAmount
												end
												...
												sourceHeal.overheal = sourceHeal.overheal + spellAmount.over
												sourceHeal.absorbed = sourceHeal.absorbed + spellAmount.absorbed
												sourceHeal.count = sourceHeal.count + spellAmount.count
												sourceHeal.crit = sourceHeal.crit + spellAmount.crit
												sourceHeal.critcount = sourceHeal.critcount + spellAmount.critcount
												sourceHeal.critmax = max(sourceHeal.critmax,spellAmount.critmax)
												sourceHeal.hitmax = max(sourceHeal.hitmax,spellAmount.hitmax)
												sourceHeal.critover = sourceHeal.critover + spellAmount.critover
												sourceHeal.absorbs = sourceHeal.absorbs + spellAmount.absorbs
												total = total + spellAmount.amount - spellAmount.over + spellAmount.absorbed
												totalOver = totalOver + spellAmount.over
	...
	local DeathTab_MissTypeLabels = {
		ABSORB  = (ACTION_SPELL_MISSED_ABSORB  and strlower(ACTION_SPELL_MISSED_ABSORB))  or "absorbed",
		BLOCK   = (ACTION_SPELL_MISSED_BLOCK   and strlower(ACTION_SPELL_MISSED_BLOCK))   or "blocked",
		DEFLECT = (ACTION_SPELL_MISSED_DEFLECT and strlower(ACTION_SPELL_MISSED_DEFLECT)) or "deflect",
		DODGE   = (ACTION_SPELL_MISSED_DODGE   and strlower(ACTION_SPELL_MISSED_DODGE))   or "dodge",
		EVADE   = (ACTION_SPELL_MISSED_EVADE   and strlower(ACTION_SPELL_MISSED_EVADE))   or "evade",
		IMMUNE  = (ACTION_SPELL_MISSED_IMMUNE  and strlower(ACTION_SPELL_MISSED_IMMUNE))  or "immune",
		MISS    = (ACTION_SPELL_MISSED_MISS    and strlower(ACTION_SPELL_MISSED_MISS))    or "miss",
		PARRY   = (ACTION_SPELL_MISSED_PARRY   and strlower(ACTION_SPELL_MISSED_PARRY))   or "parry",
		REFLECT = (ACTION_SPELL_MISSED_REFLECT and strlower(ACTION_SPELL_MISSED_REFLECT)) or "reflect",
		RESIST  = (ACTION_SPELL_MISSED_RESIST  and strlower(ACTION_SPELL_MISSED_RESIST))  or "resist",
	}
	...
	local function DeathTab_SetDeath(_,arg,arg2)
		...
		for i=1,#_data do
			...
			if row[3] then
				local skip = false
				if row[1] == 1 then
					local amount      = (row[5] or 0) - (row[6] or 0)
					local absorbedAmt = row[9] or 0
					local blockedAmt  = row[8] or 0
					local missType    = row[17]
					if amount <= 0 and absorbedAmt == 0 and blockedAmt == 0 and not missType then
						skip = true
					end
				end
				if not skip then
		...
		for i=1,#data do
			if data[i][1] then
				...
				elseif data[i][1] == 1 then
					...
					local blockedAmt = data[i][8] or 0
					local absorbedAmt = data[i][9] or 0
					local blocked  = blockedAmt > 0 and " ("..L.BossWatcherDeathBlocked..":"..blockedAmt..")" or ""
					local isCrit   = data[i][10] and "*" or ""
					local school   = " ("..GetSchoolName(data[i][7])..")"
					local amount   = data[i][5] - (data[i][6] or 0)
					local missType = data[i][17]
					local hitText
					if missType and amount <= 0 then
						local label = DeathTab_MissTypeLabels[missType] or strlower(missType)
						if missType == "ABSORB" and absorbedAmt > 0 then
							hitText = " "..L.BossWatcherDeathAbsorbed..": "..absorbedAmt
						elseif missType == "BLOCK" and blockedAmt > 0 then
							hitText = " ["..label..": "..blockedAmt.."]"
							blocked = ""
						else
							hitText = " ["..label.."]"
						end
					elseif amount <= 0 and absorbedAmt > 0 then
						hitText = " "..L.BossWatcherDeathAbsorbed..": "..absorbedAmt
					else
						hitText = " "..L.BossWatcherDeathOn.." "..isCrit..amount..isCrit
					end
					local HP = ""
					...
					local text = HP..(sourceMarker and GetTargetIconText(sourceMarker) or "")..name.." "..L.BossWatcherDeathDamage.." |T"..spellTexture..":0|t"..spellName..hitText..blocked..overkill..school
					...
					if missType and amount <= 0 then
						local label = DeathTab_MissTypeLabels[missType] or strlower(missType)
						if missType == "ABSORB" and absorbedAmt > 0 then
							reportTail = " ["..label..":"..absorbedAmt.."]"
						elseif missType == "BLOCK" and blockedAmt > 0 then
							reportTail = " ["..label..":"..blockedAmt.."]"
						else
							reportTail = " ["..label.."]"
						end
					else
						reportTail = " -"..isCrit..amount..isCrit..blocked..overkill
					end
					reportData[8][#reportData[8] + 1] = diffTime.."s."..HP..reportTail.." ["..GetGUID(data[i][2]).." - "..GetSpellLink(data[i][4]).."]"
	...
	local function DeathTab_SetDeathList()
		...
		for i,deathData in ipairs(CurrentFight.deathLog) do
			...
			local showFriendlyPlayer = isFriendly and isPlayer and DeathTab_Variables.onlyPlayers and not DeathTab_Variables.isEnemy
			local showFriendlyNpc = isFriendly and (not isPlayer) and (not DeathTab_Variables.onlyPlayers) and (not DeathTab_Variables.isEnemy)
			local showHostile = (not isFriendly) and DeathTab_Variables.isEnemy
			if (showFriendlyPlayer or showFriendlyNpc or showHostile) and (deathData[2] and deathData[2][1]) then
				counter = counter + 1
	...
	tab.timeSlider:SetScript("OnMinMaxChanged",function (self)
		...
		if tab.timeSlider.SetObeyStepOnDrag then
	...
	local function PositionsTab_RaidFrame_UpdateHP(self,segment)
		...
		self:Show()
		if self.hp_coloredFor ~= self.Unit then
			local class, matchedGUID = nil, nil
			if CurrentFight and CurrentFight.raidguids then
				for guid,nm in pairs(CurrentFight.raidguids) do
					if nm == self.Unit then
						matchedGUID = guid
						class = CurrentFight.classes and CurrentFight.classes[guid]
						break
					end
				end
			end
			if not class and matchedGUID then
				if GetUnitTypeByGUID and GetUnitTypeByGUID(matchedGUID) == 0 then
					local ok, _, c = pcall(GetPlayerInfoByGUID, matchedGUID)
					if ok then class = c end
				end
			end
			if not class and UnitClass and self.Unit then
				local _, c = UnitClass(self.Unit)
				if c then class = c end
			end
			local cc = class and ((CUSTOM_CLASS_COLORS and CUSTOM_CLASS_COLORS[class]) or (RAID_CLASS_COLORS and RAID_CLASS_COLORS[class]))
			if cc then
				self.hp:SetColorTexture(cc.r, cc.g, cc.b, 1)
				self.hp_coloredFor = self.Unit
			else
				self.hp:SetColorTexture(.15, .75, .15, 1)
			end
		end
		...
		if hpMax == 0 or hpNow <= 0 then
			self.hp:Hide()
			if self.back then self.back:Hide() end
		else
			if self.back then self.back:Show() end
			self.hp:Show()
	...
	local function PositionsTab_UnitFrame_UpdateHP(self,sec)
		...
		self:Show()
		if self.hp_coloredFor ~= self.Unit then
			self.hp:SetColorTexture(.85, .15, .15, 1)
			self.hp_coloredFor = self.Unit
		end
		self.hp:Show()
		self.hp:SetWidth(max(hpNow / hpMax * self.WIDTH,1))
	...
	local function PositionsTab_AnyFrame_OnEnter(self)
		...
		local segData = nil
		do
			local seg = posTab.timeSlider and posTab.timeSlider.GetValue and posTab.timeSlider:GetValue()
			if seg then segData = CurrentFight and CurrentFight.graphData and CurrentFight.graphData[ceil(seg/2)] end
		end
		local unitData = segData and segData[self.Unit]
		local displayName = (unitData and unitData.name) or self.Unit
		local cR,cG,cB = 1,1,1
		if CurrentFight and CurrentFight.raidguids and CurrentFight.classes then
			for guid,nm in pairs(CurrentFight.raidguids) do
				if nm == self.Unit then
					local class = CurrentFight.classes[guid]
					if class then cR,cG,cB = ExRT.F.classColorNum(class) end
					break
				end
			end
		end
		GameTooltip:AddLine(displayName, cR, cG, cB)
		if unitData then
			if unitData.hpmax and unitData.hpmax > 0 then
				GameTooltip:AddLine(format("HP: %s / %s  (%.1f%%)", tostring(unitData.health or 0), tostring(unitData.hpmax), (unitData.health or 0)/unitData.hpmax*100), 1,1,1)
			end
			if unitData.pmax and unitData.pmax > 0 then
				GameTooltip:AddLine(format("Power: %s / %s", tostring(unitData.power or 0), tostring(unitData.pmax)), .6,.6,1)
			end
		end
		GameTooltip:Show()
	end
	local function PositionsTab_AnyFrame_OnLeave(self)
		posTab.scroll.disableTooltip = nil
		GameTooltip:Hide()
	end
	...
	for i=1,8 do
		for j=1,5 do
			...
			frame.back:SetColorTexture(.35,.06,.06,1)

			frame.hp = frame:CreateTexture(nil, "BORDER")
			frame.hp:SetSize(WIDTH,HEIGHT)
			frame.hp:SetPoint("LEFT",0,0)
			frame.hp:SetColorTexture(.15,.75,.15,1)

			frame.power = frame:CreateTexture(nil, "ARTWORK")
			...
			frame:SetScript("OnEnter", PositionsTab_AnyFrame_OnEnter)
			frame:SetScript("OnLeave", PositionsTab_AnyFrame_OnLeave)
			...
			for i=1,6 do
				...
				frame.debuffs[i]:EnableMouse(true)
				if frame.debuffs[i].SetMouseClickEnabled then frame.debuffs[i]:SetMouseClickEnabled(false) end
	...
	for i=1,6 do
		...
		frame.hp:SetSize(WIDTH,HEIGHT)
		frame.hp:SetPoint("LEFT",0,0)
		frame.hp:SetColorTexture(.85,.15,.15,1)
				...
				if guid then
					...
					for k=1,#aurasData do
						...
						if aurasLine[3] == guid and aurasLine[7] == 'DEBUFF' then
							...
							local spellID = tonumber(aurasLine[6])
							if spellID and not PositionsTab_Variables.DebuffsBlackList[ spellID ] then
	...
	function tab.spellsList:HoverListValue(isHover,index,hoveredObj)
		if not isHover then
			GameTooltip_Hide()
			return
		end
		local spellID = self.D and self.D[index]
		if not spellID then return end
		GameTooltip:SetOwner(hoveredObj or self,"ANCHOR_CURSOR")
		local ok = pcall(GameTooltip.SetHyperlink, GameTooltip, "spell:"..spellID)
		if not ok then
			local n,_,t = GetSpellInfo(spellID)
			GameTooltip:SetText("|T"..(t or "Interface\\Icons\\INV_Misc_QuestionMark")..":20|t "..(n or ("spell:"..spellID)))
		end
		GameTooltip:AddLine(" ")
		GameTooltip:AddLine("SpellID: "..spellID, 0.6, 0.8, 1)
		GameTooltip:Show()
	end
	...
	trackingTab.emptyHint = ELib:Text(trackingTab.headerTab.tabs[1],
		L.BossWatcherTrackingEmptyHint,
		11
	):Size(600,80):Point("TOPLEFT",trackingTab.spellsList,"TOPRIGHT",10,-10):Color(.7,.7,.7,1):Top()
	...
	local function UpdateTrackingPage()
		...
		for i=1,#D do
			...
			if spellName then
				filteredD[#filteredD+1] = D[i]
				L[#filteredD] = "|T"..(spellTexture or "Interface\\Icons\\INV_Misc_QuestionMark")..":0|t "..spellName
			end
		end
		trackingTab.spellsList.D = filteredD
		for i=#filteredD+1,#L do L[i] = nil end

		trackingTab.spellsList:Update()
		trackingTab.targetsList:Update()
		if trackingTab.emptyHint then
			if #filteredD == 0 then trackingTab.emptyHint:Show() else trackingTab.emptyHint:Hide() end
		end
	end

	tab.optionsSpellsList = ELib:ScrollTableList(tab.headerTab.tabs[2],80,20,0,20):Size(650,400):Point(10,-15)
	function tab.optionsSpellsList:HoverListValue(isHover,index,hoveredObj)
		if not isHover then
			GameTooltip_Hide()
			return
		end
		local row = self.L[index]
		local spellID = row and row[1]
		if not spellID then return end
		GameTooltip:SetOwner(hoveredObj or self,"ANCHOR_CURSOR")
		local ok = pcall(GameTooltip.SetHyperlink, GameTooltip, "spell:"..spellID)
		if not ok then
		...
		GameTooltip:AddLine("SpellID: "..spellID, 0.6, 0.8, 1)
		if row[4] ~= "" then
			GameTooltip:AddLine(L.BossWatcherTrackingClickXToRemove or "Click the [X] at the right edge to remove this spell.", 1, 1, 0.6, true)
		else
			GameTooltip:AddLine(L.BossWatcherTrackingDefaultSpell or "Default tracked spell (cannot be removed).", 0.7, 0.7, 0.7, true)
		end
		GameTooltip:Show()
	end
	...
	tab.emptyHint = ELib:Text(tab,
		L.BossWatcherDamageTakenEmptyHint,
		11
	):Size(700,80):Point("CENTER",tab.graph,"CENTER",0,0):Color(.7,.7,.7,1):Top()
	tab.emptyHint:Hide()
	tab.UpdateInner = tab.Update
	function tab:Update()
		self:UpdateInner()
		local hasData = false
		if self.graph and self.graph.data then
			for i=1,#self.graph.data do
				local row = self.graph.data[i]
