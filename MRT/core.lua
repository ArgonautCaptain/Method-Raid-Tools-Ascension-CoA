...
MRT.V = 5004
MRT.VString = "v5.0.4"
>>>>>>> 230a266aef06be6eccc09039d574546ead211584
MRT.T = "R"
...
do
	...
	function MRT:New(moduleName,localizatedName,disableOptions)
		...
		self.main.ADDON_LOADED = MRT.NULLfunc
...
local CLEUFrame = CreateFrame("Frame", "MRTCLEUFrame", UIParent)
local CLEUList = {}
local CLEUModules = {}
local CLEUListLen = 0
...
local function CLEU_OnEvent(self, ev, ...)
	local timestamp,subEvent,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,
		val1,val2,val3,val4,val5,val6,val7,val8,val9,val10,val11,val12,val13
	if CombatLogGetCurrentEventInfo and not MRT.isClassic then
		timestamp,subEvent,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,
			val1,val2,val3,val4,val5,val6,val7,val8,val9,val10,val11,val12,val13
				= CombatLogGetCurrentEventInfo()
	else
		local sG,sN,sF,dG,dN,dF
		timestamp,subEvent,sG,sN,sF,dG,dN,dF,
			val1,val2,val3,val4,val5,val6,val7,val8,val9,val10,val11,val12,val13
				= ...
		hideCaster = false
		sourceGUID,sourceName,sourceFlags,sourceFlags2 = sG,sN,sF,0
		destGUID,destName,destFlags,destFlags2 = dG,dN,dF,0
	end
	if false and VMRT and VMRT.ExCD2 and VMRT.ExCD2.debugCLEU and subEvent == "SPELL_CAST_SUCCESS" then
		print(string.format("|cffff9900[MRT/CORE/CLEU]|r subEvent=%s src=%s spell=%s(%s) listLen=%d",
			tostring(subEvent), tostring(sourceName), tostring(val2), tostring(val1), CLEUListLen))
	end
	...
	for i=1,CLEUListLen do
		CLEUList[i](timestamp,subEvent,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,val1,val2,val3,val4,val5,val6,val7,val8,val9,val10,val11,val12,val13)
	end
end

local function CLEU_OnEvent_Recreate(self, ev, ...)
	...
	CLEU_OnEvent(self, ev, ...)
end
local function CLEU_OnEvent_RecreateProfiling(self, ev, ...)
	for i=1,#CLEUList do CLEUList[i]=nil end
	CLEUListLen = 0
	for mod,func in pairs(CLEUModules) do
		CLEUListLen = CLEUListLen + 1
...
local WOTLK_EVENT_MAP = {
	GROUP_ROSTER_UPDATE = {"RAID_ROSTER_UPDATE","PARTY_MEMBERS_CHANGED"},
	PLAYER_SPECIALIZATION_CHANGED = {"PLAYER_TALENT_UPDATE"},
	INSPECT_READY = {"INSPECT_READY","INSPECT_TALENT_READY"},
}
	...
	for i=1,select("#", ...) do
		...
		if event ~= "COMBAT_LOG_EVENT_UNFILTERED" then
			if not MRT.isClassic then
				...
				local mapped = WOTLK_EVENT_MAP[event]
				if mapped then
					if type(self.main[event]) == "function" then
						for _,alias in ipairs(mapped) do
							if not self.main[alias] then
								if alias ~= event and event == "INSPECT_READY" then
									local origHandler = self.main[event]
									self.main[alias] = function(s, arg1, ...)
										if not arg1 and _G.MRT_GetQueuedInspectGUID then
											arg1 = _G.MRT_GetQueuedInspectGUID()
										end
										return origHandler(s, arg1, ...)
									end
								else
									self.main[alias] = self.main[event]
								end
							end
							pcall(self.main.RegisterEvent,self.main,alias)
						end
					end
				else
		...
		elseif self.CLEUNotInList then
			...
			if type(func) == "function" then
				...
				CLEUFrame.__MRT_registeredModules = CLEUFrame.__MRT_registeredModules or {}
				CLEUFrame.__MRT_registeredModules[self] = self.name or true
			else
	...
	for i=1,select("#", ...) do
		...
		if event ~= "COMBAT_LOG_EVENT_UNFILTERED" then
			if not MRT.isClassic then
				...
				if mapped then
					for _,alias in ipairs(mapped) do
						pcall(self.main.UnregisterEvent,self.main,alias)
					end
				else
...
function MRT.F:GetProfiling()
	...
	local str = ""..(MRT.VString or MRT.V).." "..MRT.T.." "..now.."\n"
	local r = {}
...
if MRT.locale and MRT.locale:find("^zh") then
...
function MRT.F.SendExMsgExt(opt, ...)
	specialOpt = opt
	local args = {...}
	xpcall(function() MRT.F.SendExMsg(unpack(args)) end, geterrorhandler())
	specialOpt = nil
end
