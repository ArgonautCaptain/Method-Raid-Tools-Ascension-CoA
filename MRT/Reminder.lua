...
local GetSpellTexture = ExRT.F.GetSpellTextureSafe or GetSpellTexture
...
local function BuildSynthFromBWFight(fight)
	if type(fight) ~= "table" or not fight.encounterStart or not fight.encounterEnd then
		return nil
	end
	local tf = fight.timeFix
	local function toGT(cleuTS)
		if tf and tf[1] and tf[2] then
			return cleuTS - tf[2] + tf[1]
		end
		return cleuTS
	end

	local synth = {
		{fight.encounterStart, 3, fight.encounterID, fight.encounterName or ("Boss "..tostring(fight.encounterID or "?"))},
	}

	local events = {}
	if type(fight.cast) == "table" then
		for sourceGUID, casts in pairs(fight.cast) do
			if type(casts) == "table" then
				for k=1, #casts do
					...
					if type(cast) == "table" then
						...
						local sid = cast[2]
						local ctype = cast[3]
						local sFlags = cast[7] or 0
						if ts and sid and ctype and bit.band(sFlags, 0x3F0) == 0x240 then
							local eventName = ctype == 2 and "SPELL_CAST_START" or "SPELL_CAST_SUCCESS"
							events[#events+1] = {toGT(ts), 1, eventName, sourceGUID, "boss", sFlags, cast[6] or 0, cast[4], "", 0, cast[5] or 0, sid}
						end
					end
				end
			end
	...
	if type(fight.auras) == "table" then
		for k=1, #fight.auras do
			local aura = fight.auras[k]
			if type(aura) == "table" then
				local ts = aura[1]
				local sourceGUID = aura[2]
				local destGUID = aura[3]
				local sourceIsFriendly = aura[4]
				local sid = aura[6]
				local action = aura[8]
				if ts and sid and not sourceIsFriendly then
					local eventName
					if action == 1 then eventName = "SPELL_AURA_APPLIED"
					elseif action == 2 then eventName = "SPELL_AURA_REMOVED"
					end
					if eventName then
						events[#events+1] = {toGT(ts), 1, eventName, sourceGUID, "boss", 0x240, 0, destGUID, "", 0, 0, sid}
					end
				end
			end
		end
	...
	for k=1, #events do
		synth[#synth+1] = events[k]
	end

	if type(fight.deathLog) == "table" then
		for j=1,#fight.deathLog do
			local entry = fight.deathLog[j]
			local hdr = entry and (entry.h or entry[1])
			local ts = hdr and (hdr[3] or hdr.time)
			local sid = hdr and (hdr[5] or hdr.spellID)
			if ts and sid then
				synth[#synth+1] = {toGT(ts), 1, "SPELL_CAST_SUCCESS", nil, hdr[2], 0, 0, hdr[2], nil, 0, 0, sid}
			end
		end
	end

	synth[#synth+1] = {fight.encounterEnd, 0}
	return synth
end
module._BuildSynthFromBWFight = BuildSynthFromBWFight
...
do
	...
	local __scaleText = size < 1 and size * 1.5 or size
	if bar.text.SetScale then
		bar.text:SetScale(__scaleText)
	else
		local __f,__s,__fl = bar.text:GetFont()
		if not __f then __f,__s,__fl = GameFontNormal:GetFont() end
		if not __s or __s == 0 then __s = 12 end
		bar.text:SetFont(__f, __s * __scaleText, __fl)
	end
	local __scaleTime = size < 1 and size * 1.5 or size
	if bar.time.SetScale then
		bar.time:SetScale(__scaleTime)
	else
		local __f,__s,__fl = bar.time:GetFont()
		if not __f then __f,__s,__fl = GameFontNormal:GetFont() end
		if not __s or __s == 0 then __s = 12 end
		bar.time:SetFont(__f, __s * __scaleTime, __fl)
	end
		end
...
local function GSUB_ModNextWord(str)
	if str:find("^specIconAndClassColor") then
		...
		if name then
			...
			if role == "TANK" then
				mod = "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:0:19:22:41|t"..mod
			elseif role == "DAMAGER" then
				mod = "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:22:41|t"..mod
			elseif role == "HEALER" then
				mod = "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:1:20|t"..mod
			end
			return mod
		else
			return ""
	...
	elseif str:find("^specIcon") then
		...
		if name then
			local role = UnitGroupRolesAssigned(name)
			if role == "TANK" then
				return "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:0:19:22:41|t"..name
			elseif role == "DAMAGER" then
				return "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:22:41|t"..name
			elseif role == "HEALER" then
				return "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:1:20|t"..name
			else
				return name
			end
		else
...
function module:GetUnitRole(unit)
	...
	if role == "HEALER" then
		...
		return role, (class == "PALADIN") and "MHEALER" or "RHEALER"
	elseif role ~= "DAMAGER" then

		return role
	else
		local _,class = UnitClass(unit)
		local isMelee = (class == "WARRIOR" or class == "PALADIN" or class == "ROGUE" or class == "DEATHKNIGHT")
		if class == "DRUID" then
			isMelee = not (UnitPowerType(unit) == 8)
...
function module:CmpUnitRole(unit,roleIndex)
	...
	if roleIndex == 6 and main ~= "TANK" then
		return true
	end
end
...
function options:Load()
	...
	local function GetBossIconPathFromName(bname)
		if type(bname) ~= "string" or bname == "" then return nil end
		local key = tostring(bname):lower():gsub("[\'\226\128\153`]","")
		key = key:gsub("[^a-z0-9]+","_"):gsub("_+","_"):gsub("^_",""):gsub("_$","")
		if key == "" then return nil end
		local file = MRT.BossIconFileByKey and MRT.BossIconFileByKey[key]
		if not file then return nil end
		return "Interface\\AddOns\\"..GlobalAddonName.."\\media\\bossicons\\"..file..".tga"
	end

	local _resolveBossImageCache = {}
	local function ResolveBossImage(bossID)
		if not bossID then return nil end
		local cached = _resolveBossImageCache[bossID]
		if cached ~= nil then
			if cached == false then return nil end
			return cached
		end
		local bname = ExRT.L and ExRT.L.bossName and ExRT.L.bossName[bossID]
		local localPath = bname and GetBossIconPathFromName(bname) or nil
		if localPath then
			_resolveBossImageCache[bossID] = localPath
			return localPath
		end
		local ej = ExRT.GDB and ExRT.GDB.encounterIDtoEJ and ExRT.GDB.encounterIDtoEJ[bossID]
		if ej and EJ_GetCreatureInfo then
			local img = select(5, EJ_GetCreatureInfo(1, ej))
			if type(img) == "string" and img ~= "" then
				_resolveBossImageCache[bossID] = img
				return img
			end
		end
		_resolveBossImageCache[bossID] = false
		return nil
	end
	...
	function options.timeLine:GetRemindersList()
		...
		if timeLineData and timeLineData.p and timeLineData.p.n then
			for i=1,#timeLineData.p.n do
				local n = timeLineData.p.n[i]
				if type(n) == "number" and n < 0 and n > -10000 then
					if not bossList then bossList = {} end
					bossList[ -n ] = timeLineData.p[i]
				end
			end
		end
	...
	function self.timeLineBoss:PreUpdate()
		local _profile = VMRT and VMRT.DebugReminder and VMRT.DebugReminder.ProfilePreUpdate
		local _t0 = _profile and debugprofilestop()
		local _tHist, _tBW, _tTL, _tStatic
		wipe(self.List)
		local subMenu = {}
		local res

		local _bwRealSetValue = self.SetValue
		local function _bwSavedFightFunc(button, _arg1, _arg2, _arg3, _arg4)
			local fight = _arg1
			local tf = fight.timeFix
			local function toGT(cleuTS)
				if tf and tf[1] and tf[2] then
			...
			local synth = {
				{fight.encounterStart, 3, fight.encounterID, fight.encounterName or ("Boss "..fight.encounterID)},
			}
			...
			synth[#synth+1] = {fight.encounterEnd, 0}
			return _bwRealSetValue(button, synth, _arg2, _arg3, _arg4)
		end
		local function _historyTableCopyFunc(button, _arg1, _arg2, _arg3, _arg4)
			local copy = ExRT.F.table_copy2(_arg1)
			if copy[1] then
				copy[1][3] = select(8, GetInstanceInfo())
			end
			return _bwRealSetValue(button, copy, _arg2, _arg3, _arg4)
		end
		...
		for _, h_key in pairs({"history","historySession"}) do
			...
			for i=1,#module.db[h_key] do
				...
				if #fight > 0 and fight[1][2] == 22 then
					...
					for j=1,#fight do
						...
						elseif start and fight[j][2] == 0 then
							...
							local nf_icon = ResolveBossImage(newFight[1][3])
							local nf_iconsize = nf_icon and 32 or nil
				...
				elseif #fight > 0 and fight[1][2] == 3 then
					boss_list.icon = ResolveBossImage(fight[1][3])
					if boss_list.icon then
						boss_list.iconsize = 32
					end
				elseif #fight > 0 and fight[1][2] == 0 then
					boss_list.arg1 = fight
					boss_list.arg3 = 5
					boss_list.func = _historyTableCopyFunc
				end
				if fightLen then
		...
		if _profile then _tHist = debugprofilestop() end
		...
		if _profile then _tTL = debugprofilestop() end

		do
			local bw = MRT and MRT.A and MRT.A.BossWatcher
			local bwData = bw and bw.db and bw.db.data
			if bwData and #bwData > 0 then
				local addedAny
				for i=1,#bwData do
					local f = bwData[i]
					if f and (f.encounterID or (f.encounterName and f.encounterName ~= "")) and f.encounterStart and f.encounterEnd and f.encounterEnd > f.encounterStart then
						if not addedAny then
							...
							subMenu[#subMenu+1] = {
								text = (MRT and MRT.L and MRT.L.BossWatcher) or "Fight Log",
								isTitle = true,
							}
						end
						local fightLen = f.encounterEnd - f.encounterStart
						local text = (f.encounterName or ("Boss "..f.encounterID))..string.format(" %d:%02d", fightLen/60, fightLen%60)
						local icon = ResolveBossImage(f.encounterID)
						local iconsize = icon and 32 or nil
						subMenu[#subMenu+1] = {
							text = text,
							arg1 = f,
							arg2 = text,
							arg3 = 2,
							func = _bwSavedFightFunc,
							icon = icon,
							iconsize = iconsize,
						}
					end
		...
		if _profile then _tBW = debugprofilestop() end
		...
		if VMRT.Reminder2.TLHistory then
			...
			local _dbg = VMRT.DebugReminder
			if _dbg and _dbg.DumpTLHistory then
				local total = 0
				local perDiff = {}
				local badBossIDs = {}
				for diffID, diffData in pairs(VMRT.Reminder2.TLHistory) do
					local c = 0
					for bossID, _ in pairs(diffData) do
						c = c + 1
						total = total + 1
						if type(bossID) ~= "number" then
							badBossIDs[#badBossIDs+1] = tostring(bossID)
						end
					end
					perDiff[#perDiff+1] = string.format("diff=%s: %d", tostring(diffID), c)
				end
				print(string.format(
					"|cffffaa00[TLHistory]|r total=%d bad=%d diffs={%s}",
					total, #badBossIDs, table.concat(perDiff, ", ")
				))
				if #badBossIDs > 0 then
					print("|cffffaa00[TLHistory]|r non-numeric bossIDs: " .. table.concat(badBossIDs, ", "))
				end
				if total == 0 then
					print("|cffffaa00[TLHistory]|r empty -> ENCOUNTER_END never reached SaveLastHistory. "
						.. "Check: VMRT.Reminder2.HistoryEnabled=" .. tostring(VMRT.Reminder2.HistoryEnabled)
						.. ", in-raid pulls since last /reload."
					)
				end
			end
			local _tlhBossIDToZone = options.timeLine._bossIDToZoneCache
			if not _tlhBossIDToZone then
				_tlhBossIDToZone = {}
				for i=1,#ExRT.GDB.EncountersList do
					local z = ExRT.GDB.EncountersList[i]
					for j=2,#z do
						_tlhBossIDToZone[ z[j] ] = {z, j}
					end
				end
				options.timeLine._bossIDToZoneCache = _tlhBossIDToZone
			end

			local _zoneEntryCache = options.timeLine._tlhZoneEntryCache
			if not _zoneEntryCache then
				_zoneEntryCache = {}
			...
			for diffID,diffData in pairs(VMRT.Reminder2.TLHistory) do
				for bossID,bossData in pairs(diffData) do
					if type(bossID) == "number" then
						...
						local _cached = _tlhBossIDToZone[bossID]
						local zone = _cached and _cached[1]
						local bossNum = _cached and _cached[2]
						if zone then
							toadd = ExRT.F.table_find3(tlSubMenu,zone[1],"arg3")
							if not toadd then
								local zoneKey = zone[1]
								local zEntry = _zoneEntryCache[zoneKey]
								if not zEntry then
									local text = GetMapNameByID(zoneKey)
									local zoneImg
									local zoneMapID
									local ej_bossID = ExRT.GDB.encounterIDtoEJ[bossID]
									if ej_bossID and EJ_GetEncounterInfo then
										local _name, _description, _journalEncounterID, _rootSectionID, _link, journalInstanceID = EJ_GetEncounterInfo(ej_bossID)
										if journalInstanceID then
											local iname, _idesc, _bgImage, buttonImage1, _loreImage, _buttonImage2, _dungeonAreaMapID, _ilink, _shouldDisplayDifficulty, mapID = EJ_GetInstanceInfo(journalInstanceID)
						...
						local bossImg = ResolveBossImage(bossID)
			...
			do
				...
				local bwData = bw and bw.db and bw.db.data
				if bwData and #bwData > 0 then
					local nameToBoss = options.timeLine._tlhNameToBossCache
					if not nameToBoss then
						nameToBoss = {}
						for i=1,#ExRT.GDB.EncountersList do
							local z = ExRT.GDB.EncountersList[i]
							for j=2,#z do
								...
								if type(name) == "string" and name ~= "" and not nameToBoss[name] then
									nameToBoss[name] = {id, z, j}
								end
							end
						end
						options.timeLine._tlhNameToBossCache = nameToBoss
					end

					local _dbg2 = VMRT.DebugReminder
					local _dumpBW = _dbg2 and _dbg2.DumpBWPerBoss
					local _dumpStats = _dumpBW and {kept=0, skippedShort=0, skippedNoMatch=0, total=#bwData}

					for i=1,#bwData do
						local f = bwData[i]
						if f and f.encounterStart and f.encounterEnd
							and f.encounterEnd > f.encounterStart then
							local fightLen = f.encounterEnd - f.encounterStart

							if fightLen >= 30 then
								local bossID = type(f.encounterID) == "number" and f.encounterID or nil
								local _cached = bossID and _tlhBossIDToZone[bossID]
								local zone = _cached and _cached[1]
								local bossNum = _cached and _cached[2]

								if not zone and type(f.encounterName) == "string" and f.encounterName ~= "" then
									local hit = nameToBoss[f.encounterName]
									if hit then
										bossID = hit[1]
										zone = hit[2]
										bossNum = hit[3]
								...
								if zone or (bossID and type(bossID) == "number") or (type(f.encounterName) == "string" and f.encounterName ~= "") then
									local toadd
									if zone then
										toadd = ExRT.F.table_find3(tlSubMenu,zone[1],"arg3")
										if not toadd then
											...
											if not zEntry then
												...
												local zoneImg
												local zoneMapID
												local ej_bossID = bossID and ExRT.GDB.encounterIDtoEJ[bossID]
												if ej_bossID and EJ_GetEncounterInfo then
													local _name, _description, _journalEncounterID, _rootSectionID, _link, journalInstanceID = EJ_GetEncounterInfo(ej_bossID)
													if journalInstanceID then
														local iname, _idesc, _bgImage, buttonImage1, _loreImage, _buttonImage2, _dungeonAreaMapID, _ilink, _shouldDisplayDifficulty, mapID = EJ_GetInstanceInfo(journalInstanceID)
										...
										toadd = ExRT.F.table_find3(tlSubMenu,"other","arg3")
										if not toadd then
											toadd = {
												text = "Other",
												arg3 = "other",
									...
									local rowKey = bossID or f.encounterName
									local bossName = (bossID and ExRT.L.bossName[bossID]) or f.encounterName or (bossID and ("Boss "..bossID)) or "?"
									local toadd2 = ExRT.F.table_find3(toadd,rowKey,"arg3")
									if not toadd2 then
										local bossImg = bossID and ResolveBossImage(bossID) or nil
										toadd2 = {
											text = bossName,
											arg3 = rowKey,
											subMenu = {},
											icon = bossImg,
											iconsize = bossImg and 32 or nil,
											prio = bossNum or (bossID and (1+(bossID/100000))) or 0,
										}
										toadd[#toadd+1] = toadd2
									end
									local fightSubMenu = toadd2.subMenu

									local label = (f.encounterName or bossName)..string.format(" %d:%02d", fightLen/60, fightLen%60)
									fightSubMenu[#fightSubMenu+1] = {
										text = label,
										arg1 = f,
										arg2 = label,
										arg3 = 2,
										func = _bwSavedFightFunc,
										prio = f.encounterStartGlobal or (1e12 - i),
									}
									if _dumpStats then _dumpStats.kept = _dumpStats.kept + 1 end
								else
									if _dumpStats then _dumpStats.skippedNoMatch = _dumpStats.skippedNoMatch + 1 end
								end
							else
								if _dumpStats then _dumpStats.skippedShort = _dumpStats.skippedShort + 1 end
							end
						end
					end

					if _dumpBW and _dumpStats then
						print(string.format(
							"|cffffaa00[BWPerBoss]|r total=%d kept=%d skippedShort(<30s)=%d skippedNoMatch=%d",
							_dumpStats.total, _dumpStats.kept, _dumpStats.skippedShort, _dumpStats.skippedNoMatch
						))
						for i=1,#bwData do
							local f = bwData[i]
							if f and f.encounterStart and f.encounterEnd then
								local fightLen = f.encounterEnd - f.encounterStart
								local matched
								local rid = type(f.encounterID) == "number" and f.encounterID or nil
								if rid and _tlhBossIDToZone[rid] then matched = "id" end
								if not matched and type(f.encounterName) == "string" and nameToBoss[f.encounterName] then matched = "name" end
								print(string.format(
									"|cffffaa00[BWPerBoss]|r #%d encID=%s name=%s len=%.1fs match=%s",
									i, tostring(f.encounterID), tostring(f.encounterName), fightLen, matched or (fightLen < 30 and "shortgate" or "none")
								))
							end
						end
					end
			...
			for i=1,#tlSubMenu do
				...
				if list.zonemd then
					...
					for j=1,#list.subMenu do
						local bossEntry = list.subMenu[j]
						if bossEntry.subMenu and #bossEntry.subMenu > 0 then
							sort(bossEntry.subMenu, function(a,b) return (a.prio or 0) > (b.prio or 0) end)
						end
					end
				end
			end
		...
		local cache = self._staticBossDropdownCache
		if not cache then
			local cacheEntries = {}
			local bossEntries = {}
			...
			local listDung = {}
			cacheEntries[#cacheEntries+1] = {text = DUNGEONS or "DUNGEONS", arg3 = "dung", subMenu = listDung, prio = 40000-1, isHidden = ExRT.isClassic}

			local bossIDToZone = options.timeLine._bossIDToZoneCache
			if not bossIDToZone then
				bossIDToZone = {}
				for i=1,#ExRT.GDB.EncountersList do
					local z = ExRT.GDB.EncountersList[i]
					for j=2,#z do
						bossIDToZone[ z[j] ] = {z, j}
					end
				end
				options.timeLine._bossIDToZoneCache = bossIDToZone
			end

			local entriesByZone = {}
			local dungEntriesByZone = {}
			local mplusEntry
			...
			for bossID,bossData in pairs(options.timeLine.Data) do
				...
				local isZone
				if bossID > 0 then
					local cached = bossIDToZone[bossID]
					local zone = cached and cached[1]
					if zone then
						local isDung = dungIDs[ zone[1] ]
						local zoneTracker = isDung and dungEntriesByZone or entriesByZone
						toadd = zoneTracker[zone[1]]
						if not toadd then
							...
							if ej_bossID and EJ_GetEncounterInfo then
								...
								if journalInstanceID then
									local name2, description2, bgImage, buttonImage1, loreImage, buttonImage2, dungeonAreaMapID, link2, shouldDisplayDifficulty, mapID = EJ_GetInstanceInfo(journalInstanceID)
									zoneImg = buttonImage1
									text = name2 or text
									zoneMapID = mapID
								end
					...
					isZone = true
					if not mplusEntry then
						mplusEntry = {text = PLAYER_DIFFICULTY_MYTHIC_PLUS or "M+", arg3 = "m+", subMenu = {}, prio = 40000-2, isHidden = ExRT.isClassic}
						cacheEntries[#cacheEntries+1] = mplusEntry
					end
					toadd = mplusEntry.subMenu
				end
				if not toadd then
					toadd = cacheEntries
				end

				local bossImg = (not isZone) and ResolveBossImage(bossID) or nil
				...
				if bossData.m then
					...
					for i=1,#bossData do
						...
						if not (bossData_i and bossData_i.d) then
						else
				...
				if bossID > 0 then
					bossEntries[bossID] = boss_list
				end
			end
			for i=1,#cacheEntries do
				local list = cacheEntries[i]
				if list.zonemd then
			...
			self._staticBossDropdownCache = cache
		end

		for i=1,#cache.entries do
			self.List[#self.List+1] = cache.entries[i]
		end

		if module.db.lastEncounterID then
			local entry = cache.bossEntries[module.db.lastEncounterID]
			if entry then
				res = function() self:SetValue(entry.arg1,entry.arg2) end
			end
		elseif VMRT.Reminder2.TLBoss then
			local tlInt = floor(VMRT.Reminder2.TLBoss)
			local entry = cache.bossEntries[tlInt]
			local bossData = options.timeLine.Data[tlInt]
			if entry and bossData then
				if VMRT.Reminder2.TLBoss % 1 ~= 0 then
					local n = floor( (VMRT.Reminder2.TLBoss % 1) * 100 + 0.5 )
					local function pickTL()
						if not bossData.m then return bossData end
						local cand = bossData[n]
						if cand and cand.d then return cand end
						for i=1,#bossData do
							if bossData[i] and bossData[i].d then
								return bossData[i]
							end
						end
						return bossData
					end
					res = function() self:SetValue(tlInt,entry.arg2,3,{id = VMRT.Reminder2.TLBoss, tl = pickTL()}) end
				else
					res = function() self:SetValue(tlInt,entry.arg2) end
				end
			end
		end
		self.List[#self.List+1] = {
		...
		if _profile then
			_tStatic = debugprofilestop()
			local n_hist = #(module.db.history or {}) + #(module.db.historySession or {})
			local bw = MRT and MRT.A and MRT.A.BossWatcher
			local n_bw = bw and bw.db and bw.db.data and #bw.db.data or 0
			local n_list = #self.List
			local n_sub = 0
			for i=1,#self.List do
				local s = self.List[i].subMenu
				if s then n_sub = n_sub + #s end
			end
			print(string.format(
				"|cff33ff99[Reminder PreUpdate]|r total=%.1fms hist=%.1fms tl=%.1fms bw=%.1fms static=%.1fms | fights hist=%d bw=%d list=%d sub_total=%d",
				_tStatic - _t0,
				(_tHist or _t0) - _t0,
				(_tTL or _tHist or _t0) - (_tHist or _t0),
				(_tBW or _tTL or _tHist or _t0) - (_tTL or _tHist or _t0),
				_tStatic - (_tBW or _tTL or _tHist or _t0),
				n_hist, n_bw, n_list, n_sub
			))
		end

		local _dbg = VMRT and VMRT.DebugReminder
	...
	self.timeLineAdjustFL = ELib:Button(self.tab.tabs[4],L.ReminderAdjustFL):Point("RIGHT",self.timeLineExportToNote,"LEFT",-5,0):Size(140,20):OnClick(function(self)
		if self.subframe:IsShown() then self.subframe:Hide() else self.subframe:Show() end
	end)
	...
	self.timeLineAdjustFL.subframe:SetFrameStrata("FULLSCREEN_DIALOG")
	self.timeLineAdjustFL.subframe:SetFrameLevel((self.timeLineAdjustFL:GetFrameLevel() or 0) + 50)
	self.timeLineAdjustFL.subframe:SetScript("OnUpdate",function(self)
		if IsMouseButtonDown(1) and not self:IsMouseOver() and not self:GetParent():IsMouseOver() then
			self:Hide()
		end
	end)
	...
	options.timeLine.frame.D = CreateFrame("Frame",nil,options.timeLine.frame)
	options.timeLine.frame.D:SetAllPoints(options.timeLine.frame.C)
	options.timeLine.frame.D:SetFrameStrata("DIALOG")
	...
	options.timeLine.frame:SetScript("OnUpdate",function(self)
		...
		if self:IsMouseOver() and (not options.quickSetupFrame:IsShown() or not options.quickSetupFrame:IsMouseOver()) and not self.moveSpotted then
			...
			x = options.timeLine:GetTimeAdjust(x, true)
		...
		local viewW = self:GetWidth()
		local minX, maxX = scroll - 1, scroll + viewW + 1
		if self.runLine.__timelineX then
			local rx = self.runLine.__timelineX
			if rx < minX or rx > maxX then
				self.runLine:SetAlpha(0)
			else
				self.runLine:SetAlpha(1)
			end
		end
		local pcursors = self.pcursors
		if pcursors then
			for i = 1, #pcursors do
				local pc = pcursors[i]
				if pc and pc:IsShown() and pc.__timelineX then
					local px = pc.__timelineX
					local visible = (px >= minX and px <= maxX) and 1 or 0
					pc:SetAlpha(visible)
					if pc.text then
						pc.text:SetAlpha(visible)
					end
				end
			end
		end
	...
	if options.timeLine.frame.EnableMouse then
		options.timeLine.frame:EnableMouse(true)
	end
	...
	function options.timeLine:UpdateTimeText()
		...
		self.frame.timeLeft:SetText(module:FormatTime(self:GetTimeAdjust(x, true)))

		local x2 = self:GetTimeFromPos(self.frame:GetHorizontalScroll() + self.frame:GetWidth())
		self.frame.timeRight:SetText(module:FormatTime(self:GetTimeAdjust(x2, true)))
		...
		local minorStep = self:GetPosFromTime(5)
		local majorStep = p
		local viewLeft = self.frame:GetHorizontalScroll()
		local viewRight = viewLeft + self.frame:GetWidth()
		local gridStartTime = floor(self:GetTimeFromPos(viewLeft) / 5) * 5
		local mc = 0
		for i = self:GetPosFromTime(gridStartTime), viewRight, minorStep do
			mc = mc + 1
			local gl = self.frame.gridLines and self.frame.gridLines[mc]
			if not gl then
				self.frame.gridLines = self.frame.gridLines or {}
				gl = self.frame.C:CreateTexture(nil,"BACKGROUND",nil,-6)
				self.frame.gridLines[mc] = gl
				gl:SetWidth(1)
				gl:SetPoint("TOP",self.frame.cursorHT2,"BOTTOM",0,0)
				gl:SetPoint("BOTTOM",self.frame.cursorH,"TOP",0,0)
			end
			local timeAtTick = self:GetTimeFromPos(i)
			local rounded = floor(timeAtTick + 0.5)
			if rounded > 0 and rounded % 30 == 0 and abs(timeAtTick - rounded) < 0.5 then
				gl:SetColorTexture(.6, .6, .6, .25)
				gl:SetWidth(1)
			else
				gl:SetColorTexture(.5, .5, .5, .10)
				gl:SetWidth(1)
			end
			gl:ClearAllPoints()
			gl:SetPoint("TOP",self.frame.cursorHT2,"BOTTOM",0,0)
			gl:SetPoint("BOTTOM",self.frame.cursorH,"TOP",0,0)
			gl:SetPoint("LEFT",self.frame.C,"LEFT",i,0)
			gl:Show()
		end
		if self.frame.gridLines then
			for i = mc + 1, #self.frame.gridLines do
				self.frame.gridLines[i]:Hide()
			end
		end
	end
	...
	function options.timeLine.frame.bigBossButtons:Add(encounterID,clickFunc)
		...
		local bossImg = ResolveBossImage(encounterID)

		button.text:SetText(ExRT.L.bossName[encounterID])

		button.bg:SetTexture(bossImg or "")
	...
	do
		...
		self.quickSetupFrame.eventDD.Update = function(self)
			...
			if trigger.event == 1 then
				local sid = trigger.spellID or 0
				if ExRT.F.WarmUpSpell then ExRT.F.WarmUpSpell(sid) end
				local name = (ExRT.F.GetSpellInfoSafe and ExRT.F.GetSpellInfoSafe(sid)) or GetSpellInfo(sid)
	...
	function options.timeLine:Update()
		...
		if timeLineData then
			...
			if timeLineData.p then
				for i=1,#timeLineData.p do
					...
					if not pcursor then
						pcursor = self.frame.D:CreateTexture(nil,"OVERLAY", nil, 7)
						...
						local atlas = MRT and MRT.DiagonalGlyphAtlas
						if atlas and atlas.CreateRenderer then
							...
							pcursor.text:ClearAllPoints()
							pcursor.text:SetPoint("BOTTOMLEFT", pcursor, "TOPRIGHT", 1, -1)
							if pcursor.text.SetJustifyH then
								pcursor.text:SetJustifyH("LEFT")
							end
							if pcursor.text.SetJustifyV then
								pcursor.text:SetJustifyV("TOP")
							end
						end
					end
		...
		for i=(data_uncategorized and 0 or 1),#data_list do
			...
			local texture = "Interface\\Icons\\INV_Scroll_03"
			if i == 0 then
	...
	self.assign = {
		...
		SpellGroups_Presetup = {
			["names"] = {"Raid CD","Personals","Externals","Utility","Movement","DPS CD","AOE CC","Single CC",},
	...
	self.assignAdjustFL = ELib:Button(self.tab.tabs[5],L.ReminderAdjustFL):Point("LEFT",self.assignSettingsButton,"RIGHT",5,0):Size(140,20):OnClick(function(self)
		if self.subframe:IsShown() then self.subframe:Hide() else self.subframe:Show() end
	end)
	...
	self.assignAdjustFL.subframe:SetFrameStrata("FULLSCREEN_DIALOG")
	self.assignAdjustFL.subframe:SetFrameLevel((self.assignAdjustFL:GetFrameLevel() or 0) + 50)
	...
	options.assign.frame.D = CreateFrame("Frame",nil,options.assign.frame)
	options.assign.frame.D:SetAllPoints(options.assign.frame.C)
	options.assign.frame.D:SetFrameStrata("DIALOG")
	...
	if options.assign.frame.EnableMouse then
		options.assign.frame:EnableMouse(true)
	end
	...
	options.assign.Util_LineAssignOnEnter = function(self)
		...
		local cd
		if spell then
			cd = (self.setup and self.setup.cd) or options.assign.custom_cd[spell] or options.assign:GetSpellBaseCD(spell)
			if cd then
				options.assign.frame:ShowCD(spell,cd,(data and data.players) or (self.setup and self.setup.players))
			end
		end

		local hasSetupTooltip = false
		if self.setup then
			local setupSpell = self.setup.spell
			if setupSpell then
				GameTooltip:SetOwner(self, "ANCHOR_LEFT")
				hasSetupTooltip = true
				if ExRT.F.WarmUpSpell then
					ExRT.F.WarmUpSpell(setupSpell)
				end
				GameTooltip:SetHyperlink("spell:"..setupSpell)
				local left = _G["GameTooltipTextLeft1"]
				local text = left and left:GetText()
				if not text or text == "" then
					GameTooltip:ClearLines()
					local name = (ExRT.F.GetSpellInfoSafe and ExRT.F.GetSpellInfoSafe(setupSpell)) or GetSpellInfo(setupSpell)
					GameTooltip:AddLine(name or ("Spell "..tostring(setupSpell)), 1, 1, 1)
					GameTooltip:AddLine("ID: "..tostring(setupSpell), 0.7, 0.7, 0.7)
				end
				if not self.funcOnEnter then
					local cdToShow = self.setup.cd or cd
					if cdToShow then
						GameTooltip:AddLine("CD: "..module:FormatTime(cdToShow))
					else
						GameTooltip:AddLine("CD: ?", 1, 0.6, 0.4)
					end
					local charges = options.assign.custom_charges[setupSpell] or self.setup.charges or 1
					GameTooltip:AddLine("Charges: "..tostring(charges))
				end
			elseif self.setup.cd then
				GameTooltip:SetOwner(self, "ANCHOR_LEFT")
				hasSetupTooltip = true
		...
		if not data then
			if hasSetupTooltip then
				GameTooltip:Show()
			end
			return
		end
		if not hasSetupTooltip then
			GameTooltip:SetOwner(self, "ANCHOR_LEFT")
		end
		local p,pc,pd
		local dt = module:ConvertMinuteStrToNum(data.triggers[1].delayTime)
	...
	function options.assign:Util_CreateLineAssign(parent)
		...
		a.icon:SetTexture("Interface\\Icons\\INV_Misc_QuestionMark")

		a.text = ELib:Text(a,"Myname",8):Point("TOPLEFT",a.icon,"TOPRIGHT",2,-2):Point("BOTTOMRIGHT",a,-2,2):Color(1,1,1):Shadow(true)
	...
	local function pickAssignTextColor(r, g, b)
		local L = 0.299 * (r or 0) + 0.587 * (g or 0) + 0.114 * (b or 0)
		if L > 0.55 then
			return 0, 0, 0
		end
		return 1, 1, 1
	...
	function options.assign:Util_LineAssignUpdateFromData(data,isSetup)
		...
		for j=1,#module.datas.rolesList do
			if data["role"..j] and module.datas.rolesList[j][5] then
				local atlasKey = module.datas.rolesList[j][5]
				local mainRole = module.datas.rolesList[j][3]
				local roleKey
				if type(atlasKey) == "string" and atlasKey:find("tank",1,true) then
					roleKey = "tank"
				elseif type(atlasKey) == "string" and atlasKey:find("healer",1,true) then
					roleKey = "healer"
				elseif type(atlasKey) == "string" and atlasKey:find("dps",1,true) then
					roleKey = "dps"
				elseif mainRole == "TANK" then
					roleKey = "tank"
				elseif mainRole == "HEALER" or mainRole == "RHEALER" or mainRole == "MHEALER" then
					roleKey = "healer"
				else
					roleKey = "dps"
				end
				...
				if roleKey == "tank" then
					a = "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:0:19:22:41|t"
				elseif roleKey == "healer" then
					a = "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:1:20|t"
				else
					a = "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:22:41|t"
				end
		...
		if multicolor then
			...
			local ar, ag, ab, n = 0, 0, 0, 0
			for i=1,#multicolor do
				local mc = multicolor[i]
				ar = ar + (mc.r or 0)
				ag = ag + (mc.g or 0)
				ab = ab + (mc.b or 0)
				n = n + 1
			...
			if n > 0 then
				self.text:Color(pickAssignTextColor(ar/n, ag/n, ab/n))
			else
		...
		elseif color then
			...
			ELib:Border(self.bg,0)
			self.text:Color(pickAssignTextColor(color.r, color.g, color.b))
			ELib:Gradient(self)
		else
	...
	function options.assign:Util_AsignOnDragStop()
		...
		local draggingSrc = options.assign.frame.dragging
		if not draggingSrc then
			if self and self.StopMovingOrSizing then self:StopMovingOrSizing() end
			if self and self.Hide then self:Hide() end
			options.assign.frame.draggingNow = nil
			options.assign.frame.draggingData = nil
			options.assign.frame.draggingCopy = nil
			options.assign.frame.draggingShowShift = nil
			if options.assign.Util_LineAssignRemoveSpace then
				options.assign:Util_LineAssignRemoveSpace()
			end
			if options.assign.frame.ShowCD then
				options.assign.frame:ShowCD(nil,nil,nil,"DRAGGING")
			end
			return
		end
		draggingSrc:_SetAlpha(1)
		draggingSrc.SetAlpha = draggingSrc._SetAlpha
	...
	options.assign.frame.quick.CDFrame:SetSize(140,40)
	options.assign.frame.quick.CDFrame:Hide()
	options.assign.frame.quick.CDFrame:SetFrameStrata("DIALOG")
	...
	options.assign.frame.quick.CDFrame.icon:SetPoint("TOPLEFT",4,-4)
	options.assign.frame.quick.CDFrame.icon:SetSize(16,16)

	options.assign.frame.quick.CDFrame.CDLabel = ELib:Text(options.assign.frame.quick.CDFrame,"CD:",9):Point("TOPLEFT",24,-2):Color(1,.82,0,1):Left()
	options.assign.frame.quick.CDFrame.ChargesLabel = ELib:Text(options.assign.frame.quick.CDFrame,"Charges:",9):Point("TOPLEFT",24,-20):Color(1,.82,0,1):Left()

	options.assign.frame.quick.CDFrame.CD = ELib:Text(options.assign.frame.quick.CDFrame,"",9):Point("TOPLEFT",80,-2):Color(1,1,1,1):Left()
	options.assign.frame.quick.CDFrame.Charges = ELib:Text(options.assign.frame.quick.CDFrame,"",9):Point("TOPLEFT",80,-20):Color(1,1,1,1):Left()
	...
	function options.assign:Util_AssignSetupOnEnter()
		...
		local cdResolved = self.setup.cd or options.assign.custom_cd[spell] or options.assign:GetSpellBaseCD(spell)
		if cdResolved then
			options.assign.frame.quick.CDFrame.CD:SetText( module:FormatTime(cdResolved) )
		else
			options.assign.frame.quick.CDFrame.CD:SetText( "?" )
		end
		options.assign.frame.quick.CDFrame.Charges:SetText( tostring(options.assign.custom_charges[spell] or self.setup.charges or 1) )
	...
	local roleDefs = {
		{"TANK",    ExRT.isClassic and "LFG-TANK"   or "UI-Frame-TankIcon",   0,      19/64,  22/64,  41/64 },
		{"HEALER",  ExRT.isClassic and "LFG-HEALER" or "UI-Frame-HealerIcon", 20/64,  39/64,   1/64,  20/64 },
		{"DAMAGER", ExRT.isClassic and "LFG-DPS"    or "UI-Frame-DpsIcon",    20/64,  39/64,  22/64,  41/64 },
	}
	for i=1,#roleDefs do
		local role = roleDefs[i]
		local b = CreateFrame("Button",nil,options.assign.frame.quick.filter)
		...
		if ExRT.isClassic then
			...
			b.icon:SetTexCoord(role[3], role[4], role[5], role[6])
		else
	...
	options.assign.frame.quick.filter.editgroupsbut = ELib:ButtonIcon(options.assign.frame.quick.filter,ExRT.isClassic and "charactercreate-icon-customize-speechbubble" or "GM-icon-settings-hover",true):Size(20,20):IconSize(ExRT.isClassic and -4 or 2):Point("TOPRIGHT",-5,-55):OnClick(function() options.assign.frame.quick.edit:Show() end):Tooltip("Edit spell groups"):VisualHover()
	...
	local _spellGroupNameMigrationV2 = {
		["raid cd"]   = "Raid CD",
		["personals"] = "Personals",
		["externals"] = "Externals",
		["ultility"]  = "Utility",
	...
	function options.assign:MigrateSpellGroupNames()
		local sg = VMRT.Reminder2 and VMRT.Reminder2.SpellGroups
		if not sg or not sg.names then return end
		if not sg.namesMigratedV2 then
			for i=1,#sg.names do
				local current = sg.names[i]
				if type(current) == "string" then
					local mapped = _spellGroupNameMigrationV2[current:lower()]
					if mapped and current ~= mapped then
						sg.names[i] = mapped
					end
				end
			end
			sg.namesMigratedV2 = true
		end
		if not sg.namesToppedUpV1 then
			local preset = options.assign.SpellGroups_Presetup and options.assign.SpellGroups_Presetup.names
			if type(preset) == "table" then
				local present = {}
				for i=1,#sg.names do
					if type(sg.names[i]) == "string" then
						present[sg.names[i]:lower()] = true
					end
				end
				for i=1,#preset do
					if not present[preset[i]:lower()] then
						local ni = #sg.names + 1
						sg.names[ni] = preset[i]
						if type(sg[ni]) ~= "table" then
							sg[ni] = {}
						end
					end
				end
			end
			sg.namesToppedUpV1 = true
		end
	end
	...
	function options.assign.frame.quick.filter:Update()
		...
		options.assign:MigrateSpellGroupNames()
	...
	function options.assign.frame.quick.edit:line_onenter()
		...
		if not text or text == "" then
			GameTooltip:ClearLines()
			local name = (ExRT.F.GetSpellInfoSafe and ExRT.F.GetSpellInfoSafe(self.spell)) or GetSpellInfo(self.spell)
			GameTooltip:AddLine(name or ("Spell "..tostring(self.spell)), 1, 1, 1)
			GameTooltip:AddLine("ID: "..tostring(self.spell), 0.7, 0.7, 0.7)
		end
		local cdResolved = options.assign.custom_cd[self.spell] or options.assign:GetSpellBaseCD(self.spell)
		if cdResolved then
			GameTooltip:AddLine("CD: "..module:FormatTime(cdResolved))
		else
			GameTooltip:AddLine("CD: ?", 1, 0.6, 0.4)
		end
		local chargesResolved = options.assign.custom_charges[self.spell] or 1
		GameTooltip:AddLine("Charges: "..tostring(chargesResolved))
		GameTooltip:Show()
	end
		...
		for i=1,#spellsList do
			...
			if data.pos + 25 >= pos and data.pos <= pos+self.frame:GetHeight() then
				...
				if ExRT.F.GetSpellInfoSafe then
					name,_,texture = ExRT.F.GetSpellInfoSafe(data.spell)
				else
					name,_,texture = GetSpellInfo(data.spell)
				end
				if (not texture or texture == "") and ExRT.F.GetSpellTextureSafe then
					texture = ExRT.F.GetSpellTextureSafe(data.spell)
				end
				...
				for j=1,#groups do
					...
					if not gbutton then
						...
						if gbutton.SetPropagateMouseClicks then
	...
	function options.assign.frame.quick.edit:Update()
		...
		for i=1,#AllSpells do
			...
			if ExRT.GDB.ClassID[class or 0] then
				...
				local sortName = GetSpellName(data[1])
				if (not sortName or sortName == "") and ExRT.F.GetSpellInfoSafe then
					sortName = ExRT.F.GetSpellInfoSafe(data[1])
				end
				self.spellsList[#self.spellsList + 1] = {
					spell = data[1],
					pos = (names_len + 1)*25 + 5 + 25 + 25 * #self.spellsList,
					class = class,
					spellName = sortName or tostring(data[1] or 0),
	...
	options.assign.frame.quick.edit.frame:SetScript("OnMouseWheel", function(self, delta)
		local step = (self.mouseWheelRange or 20) * delta
		if IsShiftKeyDown() and self.ScrollBarHorizontal then
			local cur = self.ScrollBarHorizontal:GetValue() or 0
			local lo, hi = self.ScrollBarHorizontal.slider:GetMinMaxValues()
			local target = cur - step
			if target < lo then target = lo end
			if target > hi then target = hi end
			self.ScrollBarHorizontal:SetValue(target)
		else
			local cur = self.ScrollBar:GetValue() or 0
			local lo, hi = self.ScrollBar.slider:GetMinMaxValues()
			local target = cur - step
			if target < lo then target = lo end
			if target > hi then target = hi end
			...
			if eventDB then
				if event == 1 then
					...
					if trigger.spellID then
						if ExRT.F.WarmUpSpell then ExRT.F.WarmUpSpell(trigger.spellID) end
						local spellName = (ExRT.F.GetSpellInfoSafe and ExRT.F.GetSpellInfoSafe(trigger.spellID)) or GetSpellInfo(trigger.spellID)
						local spellTexture = GetSpellTexture(trigger.spellID)
						spellText = " "
		...
		if level == 1 then
			...
			if data.bossID then
				...
				if localPath then
					...
					button.bossImg:SetTexture(localPath)
					resetBossImg = false
				elseif ExRT.GDB.encounterIDtoEJ[data.bossID] and EJ_GetCreatureInfo then
					local displayInfo = select(4, EJ_GetCreatureInfo(1, ExRT.GDB.encounterIDtoEJ[data.bossID]))
					if type(displayInfo) == "number" and displayInfo > 0 and SetPortraitTextureFromCreatureDisplayID then
						button.bossImg:SetTexCoord(0,1,0,1)
	...
	function self:GetNewUID()
			...
			local sid,pid = "0","0"
			if guid:find("-") then
				local _,a,b = strsplit("-",guid)
				sid = a or "0"
				pid = b or guid
			else
				pid = guid ~= "" and guid or (UnitName("player") or "0")
			end
			pid = tostring(pid or "0")
	...
	local ROLE_COLS = 4
	local ROLE_COL_W = 120
	local ROLE_TEXT_W = 95
	self.setupFrame.rolesChecks = {}
	for i=1,#module.datas.rolesList do
		local chk = ELib:Check(self.setupFrame.tab.tabs[3],module.datas.rolesList[i][2]):Point("LEFT",10+((i-1)%ROLE_COLS)*ROLE_COL_W,0):Point("TOP",self.setupFrame.notePatternEdit,"BOTTOM",0,-10-floor((i-1)/ROLE_COLS)*25):OnClick(function(self)
			if self:GetChecked() then
		...
		chk.text:SetWidth(ROLE_TEXT_W)
		chk.text:SetJustifyH("LEFT")
		if chk.text.SetWordWrap then chk.text:SetWordWrap(false) end
	...
	local CLASS_COLS = ROLE_COLS
	local CLASS_COL_W = ROLE_COL_W
	local CLASS_TEXT_W = ROLE_TEXT_W
	self.setupFrame.classChecks = {}
	for j=1,#ExRT.GDB.ClassList do
		local i = ((j - 1) % CLASS_COLS) + 1
		local class = ExRT.GDB.ClassList[j]
		local className = L.classLocalizate[class]
		local classColor = RAID_CLASS_COLORS[class] and RAID_CLASS_COLORS[class].colorStr and "|c"..RAID_CLASS_COLORS[class].colorStr or ""
		local chk = ELib:Check(self.setupFrame.tab.tabs[3],classColor..className):Point("LEFT",10+(i-1)*CLASS_COL_W,0):Point("TOP",self.setupFrame.rolesChecks[1],"BOTTOM",0,-10-25-25*floor((j-1)/CLASS_COLS)):OnClick(function(self)
			if self:GetChecked() then
		...
		chk.text:SetWidth(CLASS_TEXT_W)
		chk.text:SetJustifyH("LEFT")
		if chk.text.SetWordWrap then chk.text:SetWordWrap(false) end
	...
	ELib:DecorationLine(self.setupFrame.tab.tabs[3]):Point("TOP",self.setupFrame.classChecks[1],"BOTTOM",0,-5-25*ceil((#ExRT.GDB.ClassList)/CLASS_COLS)):Point("LEFT",self.setupFrame,0,0):Point("RIGHT",self.setupFrame,0,0):Size(0,1)

	self.setupFrame.neverCheck = ELib:Check(self.setupFrame.tab.tabs[3],L.ReminderDisable..":"):Point("LEFT",self.setupFrame.allPlayersCheck,"LEFT",0,0):Point("TOP",self.setupFrame.classChecks[1],"BOTTOM",0,-10-25*(ceil(#ExRT.GDB.ClassList/CLASS_COLS)-1)-25-25):Left(5):OnClick(function(self)
		if self:GetChecked() then
	...
	local function GetTriggerButton(triggerNum)
		...
		button.spellID = ELib:Edit(button.sub):Size(270,20):Point("TOPLEFT",button.targetRole,"BOTTOMLEFT",0,-5):LeftText(L.ReminderSpellID..":"):OnChange(function(self,isUser)
			...
			if not spellID then
				...
				if ExRT.F.WarmUpSpell then ExRT.F.WarmUpSpell(spellID) end
				local spellName = (ExRT.F.GetSpellInfoSafe and ExRT.F.GetSpellInfoSafe(spellID)) or GetSpellInfo(spellID)
				local spellTexture = GetSpellTexture(spellID)
	...
	function timelineFrame.filterDropDown:PreUpdate()
		...
		local subMenu = {}
		for _, h_key in pairs({"history","historySession"}) do
			local addedFromBucket = 0
			for i=1,#module.db[h_key] do
				local fight = module.db[h_key][i]
				if type(fight) ~= "table" or #fight <= 1 then
				else
					if addedFromBucket == 0 and #subMenu > 0 then
						subMenu[#subMenu+1] = {
							text = " ",
							isTitle = true,
						}
					...
					addedFromBucket = addedFromBucket + 1
				end
			end
		end
		do
			...
			if bwData and #bwData > 0 then
				...
				for i=1,#bwData do
					local f = bwData[i]
					if type(f) == "table" and (f.encounterID or (f.encounterName and f.encounterName ~= "")) and f.encounterStart and f.encounterEnd and f.encounterEnd > f.encounterStart then
						if not addedAny then
							addedAny = true
							if #subMenu > 0 then
								subMenu[#subMenu+1] = {
						...
						local fightLen = f.encounterEnd - f.encounterStart
						local text = (f.encounterName or ("Boss "..tostring(f.encounterID or "?")))..format(" %d:%02d", fightLen/60, fightLen%60)
						subMenu[#subMenu+1] = {
							text = text,
							arg1 = f,
							func = function(_,arg1)
								ELib:DropDownClose()
								local synth = BuildSynthFromBWFight(arg1)
								if synth then
									timelineFrame:Update(synth)
								end
							end,
						}
					end
		...
		if #subMenu == 0 then
			subMenu[#subMenu+1] = {
				text = "|cff888888"..(L.ReminderFightSaved or "Saved fights")..": "..(NONE or "none").."|r",
				isTitle = true,
			}
		end
		subMenu[#subMenu+1] = {
	...
	local function SpellIconButton_OnClick(self,button)
		...
		local LCG = LibStub("LibCustomGlow-1.0-MRT",true)
		if LCG then
			...
			if hline[2] == 1 then
				...
				then
					...
					if not icon then
						...
						icon.linetoline = timelineContent:CreateTexture(nil, "BACKGROUND")
					...
					if not spellTexture and ExRT.F.GetSpellTextureSafe then
						spellTexture = ExRT.F.GetSpellTextureSafe(hline[12])
					end
					icon.texture:SetTexture(spellTexture or "Interface\\Icons\\INV_Misc_QuestionMark")
					icon:Show()
	...
	self.disableChat = ELib:Check(self.options_tab.tabs[1],L.ReminderDisableChat..":",VMRT.Reminder2.disableChat):Point("TOPLEFT",self.disablePopups,"BOTTOMLEFT",0,-5):Left(5):Tooltip(L.ReminderDisableChatTooltip):OnClick(function(self)
		VMRT.Reminder2.disableChat = self:GetChecked()
	end)
	...
	do
		...
		for i = 1, #fontMedia do
			local entry = fontMedia[i]
			self.dropDownBarFont.List[i] = {
				text = entry.name,
				arg1 = entry.path,
				func = dropDownBarFontSetValue,
				font = entry.path,
				justifyH = "CENTER",
			}
		end
	...
	if C_VoiceChat and C_VoiceChat.GetTtsVoices then
		...
		function self.voicesList:Update()
			...
			local voiceID = module:GetTTSVoiceID()
			for i=1,#voices do
		...
		local function _ttsGetDefaultVoice()
			if type(TextToSpeech_GetSelectedVoice) == "function" and Enum and Enum.TtsVoiceType then
				local ok, res = pcall(TextToSpeech_GetSelectedVoice, Enum.TtsVoiceType.Standard)
				if ok and type(res) == "table" then return res.voiceID end
			end
			return 0
		end
		local function _ttsGetRate()
			if type(C_TTSSettings) == "table" and type(C_TTSSettings.GetSpeechRate) == "function" then
				local ok, r = pcall(C_TTSSettings.GetSpeechRate)
				if ok then return r end
			end
			return 0
		end
		local function _ttsGetVolume()
			if type(C_TTSSettings) == "table" and type(C_TTSSettings.GetSpeechVolume) == "function" then
				local ok, v = pcall(C_TTSSettings.GetSpeechVolume)
		...
		function self.voicesList.func_SetValue(_,arg1)
			...
			if not (C_VoiceChat and type(C_VoiceChat.SpeakText) == "function") then return end

			C_VoiceChat.SpeakText(
				arg1 or _ttsGetDefaultVoice(),
				TEXT_TO_SPEECH_SAMPLE_TEXT,
				(Enum and Enum.VoiceTtsDestination and Enum.VoiceTtsDestination.QueuedLocalPlayback) or 1,
				VMRT.Reminder2.ttsSpeechRate or _ttsGetRate(),
				VMRT.Reminder2.ttsVolume or _ttsGetVolume()
			)
		end
...
function module:DeactivateTrigger(trigger, uid, isScheduled, printLog)
	...
	if trigger.untimed and trigger.units then
...
do
	...
	function module:ShowReminderVisual(trigger,data,reminder,params)
		...
		if remType == REM.TYPE_CHAT then
			if VMRT.Reminder2 and VMRT.Reminder2.disableChat then
				return
			end
		...
		elseif remType == REM.TYPE_BAR then
			...
			if data.countdownVoice and reminderDuration ~= 0 and reminderDuration >= 1.3 then
				...
				if soundTemplate then
					...
					t.voice = clist
				elseif data.countdownVoice == -9999 then
					for i=1,min(5,reminderDuration-0.3) do
						local sound = i
...
function module.main.COMBAT_LOG_EVENT_UNFILTERED(timestamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellID,spellName,school,arg1,arg2)
	...
	if IsHistoryEnabled and CLEUIsHistoryEvent[event] and bit_band(sourceFlags,0x000003F0) == 0x00000240 then
	...
	local function BigWigsEventCallback(event, ...)
		...
		if (event == "BigWigs_Message") then
			...
			if key == "stages" and type(text)=='string' and module.db.eventsToTriggers.BOSS_PHASE then
				...
				if not isSoonAnnounce and false then
...
do
	...
	local function DBMEventCallback(event, ...)
		...
		elseif event == "DBM_SetStage" then
			...
			if stageTotal == nil and type(encounterId) == "number" then
				stageTotal = encounterId
			end
			if stage then
				...
				for i=1,#triggers do
					...
					then
						if not trigger.statuses[guid] then
							...
							if auraData.rem_changed_dur then
...
function module:SetProfile(profile,sharedProfile)
	if sharedProfile == "x" and type(VMRT.Reminder2.ProfileShared)=="number" and VMRT.Reminder2.ProfileShared >= 0 and not VMRT.Reminder2.data[VMRT.Reminder2.ProfileShared] then
		sharedProfile = 0
	end
...
function module:SaveHistorySegmentIsMatchDiff(difficultyID)
	...
	if difficultyID == 16 or difficultyID == 15 or difficultyID == 14
		or difficultyID == 194 or difficultyID == 193
		or difficultyID == 175 or difficultyID == 176
		or difficultyID == 1 or difficultyID == 2
		or difficultyID == 3 or difficultyID == 4
	then
		return true
	end
end
...
function module:SaveHistorySegment(ignoreFightLen, difficultyID, isKill)
	local _trace = VMRT and VMRT.DebugReminder and VMRT.DebugReminder.TraceEncounterBridge
	local _tlog = _G.MRT_TraceEncounterBridgeLog
	if not IsHistoryEnabled and _trace then
		local emit = _tlog or print
		emit("|cffffaa00[Reminder]|r SaveHistorySegment IsHistoryEnabled=false (HistoryEnabled=" ..
			tostring(VMRT.Reminder2 and VMRT.Reminder2.HistoryEnabled) .. ", encounter never started recording)")
	end
	if IsHistoryEnabled then
		module:AddHistoryRecord(0)
		...
		if _trace then
			local n = #module.db.historyNow
			local span = n > 1 and (module.db.historyNow[n][1] - module.db.historyNow[1][1]) or 0
			local emit = _tlog or print
			emit(string.format("|cffffaa00[Reminder]|r SaveHistorySegment diff=%s isKill=%s recs=%d span=%.1fs enoughLength=%s diffMatch=%s",
				tostring(difficultyID), tostring(isKill), n, span,
...
function module:SaveLastHistory(history, difficultyID, isKill)
	...
	if not history then
		if _trace then
			print("|cffffaa00[Reminder]|r SaveLastHistory bail: history=nil")
		end
		return
	end
	local customtl,bossID,len = module:CreateCustomTimelineFromHistory(history)
	if _trace then
		print(string.format("|cffffaa00[Reminder]|r SaveLastHistory bossID=%s len=%s diff=%s isKill=%s",
			tostring(bossID), tostring(len), tostring(difficultyID), tostring(isKill)))
	end

	local diffConverted = 3
	if difficultyID == 16 or difficultyID == 194 or difficultyID == 193 then
		diffConverted = 4
	elseif difficultyID == 14 or difficultyID == 175 or difficultyID == 176
		or difficultyID == 1 or difficultyID == 2 then
		diffConverted = 2
...
function module:CreateCustomTimelineFromHistory(fight)
	...
	local bossID = fight[1] and fight[1][3]
	if type(bossID) ~= "number" then
		for i=1,#fight do
			local entry = fight[i]
			if entry and entry[2] == 3 and type(entry[3]) == "number" then
				bossID = entry[3]
				break
			end
		end
	end
	return data, bossID, #fight > 1 and fight[#fight][1] - fight[1][1]
end
		...
		elseif subprefix == "S" then
			if arg1 == "E" and module.IsEnabled then
				...
				if arg2 == "P" then
					...
					if module.db.requestEncounterID and ( GetTime() - module.db.requestEncounterID < 5 ) and zoneID == arg3 and not module.db.encounterID then
...
function module:Test_BW(phase)
	if not BigWigs then
		LibDBIcon10_BigWigs:GetScript("OnClick")(LibDBIcon10_BigWigs,"RightButton")
