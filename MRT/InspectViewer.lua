...
local GetItemInfo, GetItemInfoInstant, GetItemQualityColor = GetItemInfo, GetItemInfoInstant, GetItemQualityColor
...
function module.options:Load()
	...
	self.chkItemsTrack.Button:SetScript("OnClick",function (this)
		if ExRT.lib.ScrollDropDown.IsOpen() then
			ELib:DropDownClose()
		else
	...
	function module.options.ReloadPage()
		local inspectMod = ExRT.A and ExRT.A.Inspect
		if inspectMod and inspectMod.PruneNonRaidMembers then
			inspectMod.PruneNonRaidMembers()
		end
			...
			if (not module.db.filter or (data and (
			  ...
			  (module.db.filterType == 4 and module.db.filter:find( "_"..(data.class or "unknown") ))
				...
				if data then
					...
					if cp then
						line.class.texture:SetTexture(cp)
						line.class.texture:SetTexCoord(cl,cr,ct,cb)
					else
					...
					if module.db.page == 1 then
						...
						if items then
							for j=1,#module.db.itemsSlotTable do
								...
								if item then
									if not item:find("|c", 1, true) then
									...
									local _,fullLink,itemQuality,itemLevel,_,_,_,_,_,itemTexture = GetItemInfo(item)
									if not itemTexture then
										local _,_,_,_,t = GetItemInfoInstant(item)
										itemTexture = t
									end
									icon.texture:SetTexture(itemTexture)
									icon.link = (type(fullLink) == "string" and fullLink:find("|H", 1, true)) and fullLink or item
									...
									local col = itemColor or "ffffffff"
		if type(col)=="string" and col:sub(1,2)=="|c" then
			icon.text:SetText(col..(itemLevel or ""))
		else
			icon.text:SetText("|c"..col..(itemLevel or ""))
		end
									...
									local isSlotForEnchant =
										...
										(slotID == 8 and (module:GetSpecMainStat(spec)=="agi" and IS_SL)) or
										(slotID == 9 and (module:GetSpecMainStat(spec)=="int" and IS_SL)) or
										(slotID == 10 and (module:GetSpecMainStat(spec)=="str" and IS_SL)) or
										(slotID == 5 and IS_SL)
			...
			local it = 1
							...
							while tree do
								...
								if spellID and spellID ~= 0 then
									...
									local texture = ExRT.F.GetSpellTextureSafe and ExRT.F.GetSpellTextureSafe(spellID) or select(3,GetSpellInfo(spellID))

									icon.texture:SetTexture(texture)
									icon.link = ExRT.F.GetSpellLinkSafe and ExRT.F.GetSpellLinkSafe(spellID) or GetSpellLink(spellID)
									icon.sid = spellID
		...
		module.options.talentsScrollFrame:SetShown(false)
		module.options.ScrollBar:SetShown(true)
	end
