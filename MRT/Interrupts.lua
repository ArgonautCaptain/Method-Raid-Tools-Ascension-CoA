...
function module.options:Load()
	...
	if not ADVANCED_LABEL then
		ADVANCED_LABEL = (L and (L.Advanced or L.advanced)) or "Advanced"
	end
	...
	do
		...
		for i = 1, #fontMedia do
			local entry = fontMedia[i]
			self.dropDownKickerFont.List[i] = {
				text = entry.name,
				arg1 = entry.path,
				func = dropDownKickerFontSetValue,
				font = entry.path,
				justifyH = "CENTER",
			}
		end
	...
	do
		local entries = {}
		for i = 1, #sounds do
			entries[#entries+1] = {name = sounds[i][2], path = sounds[i][1]}
		end
		for key, sound in ExRT.F.IterateMediaData("sound") do
			entries[#entries+1] = {name = key, path = sound}
		end
		sort(entries, function(a, b) return a.name:lower() < b.name:lower() end)
		for i = 1, #entries do
			self.dropDownSoundNext.List[i] = {
				text = entries[i].name,
				arg1 = entries[i].path,
				func = dropDownSoundNextSetValue,
			}
		end
	end
	...
	do
		...
		sort(entries, function(a, b) return a.name:lower() < b.name:lower() end)
		for i = 1, #entries do
			self.dropDownSoundKick.List[i] = {
				text = entries[i].name,
				arg1 = entries[i].path,
				func = dropDownSoundKickSetValue,
			}
		end
	end
...
function module:UpdateFrame2(frame,counter,isCasting,isMine)
	if not frame then
		return
	end
